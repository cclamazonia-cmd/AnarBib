...
// Script Node.js .cjs (UTF-8 sans BOM)
// Usage : node create-paquet-B3.cjs
// ============================================================================
// Cree :
//   supabase/migrations/20260517210000_paquet_B3_rpc_metier.sql
// ============================================================================
...
const migName  = '20260517210000_paquet_B3_rpc_metier.sql';
const migPath  = path.join(migDir, migName);

if (!fs.existsSync(migDir)) {
  console.error(`ERREUR : dossier migrations introuvable a ${migDir}`);
  process.exit(1);
}
if (fs.existsSync(migPath)) {
  console.error(`ATTENTION : la migration existe deja a ${migPath}`);
  process.exit(1);
}
...
fs.writeFileSync(migPath, sql, { encoding: 'utf8' });

const stat = fs.statSync(migPath);
...
console.log('');
console.log('============================================================');
console.log('  Migration B.3 creee avec succes');
console.log('============================================================');
console.log(`  Fichier : ${migPath}`);
console.log(`  Taille  : ${stat.size} octets`);
console.log(`  Lignes  : ${lineCount}`);
console.log(`  BOM     : ${hasBom ? 'PRESENT (ANOMALIE)' : 'absent (conforme)'}`);
console.log('');
console.log('Contenu :');
console.log('  - 1 helper public.fn_library_active_staff_count(uuid)');
console.log('  - 4 RPC metier : fn_propose / fn_vote / fn_cancel / fn_execute');
console.log('  - DO-block 6 verifications (presence + GRANTs + SECURITY DEFINER + tables vides)');
console.log('');
console.log('Prochaines etapes :');
console.log(`  1. git add supabase/migrations/${migName}`);
console.log(`  2. git commit -m "paquet B.3 : RPC metier cycle de vie propositions transitions"`);
