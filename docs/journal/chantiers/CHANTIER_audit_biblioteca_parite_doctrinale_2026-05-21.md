...
**Statut chantier-cadre** : ✅ **CLOS le 08/06/2026** — étape 8 (mails EA-13/14/19) livrée et vérifiée de bout en bout ; étapes 9-10 (parité HTML radicale) reclassées hors chantier (cf. §6.4). Décision normative actée au REGISTRE.
...
| EA-20 | identity | **Éditeur de profil de contact de bibliothèque absent.** La table `library_contact_profiles` (champs `contact_name`, `contact_role`, `contact_email`, `contact_phone`, `contact_whatsapp`, `contact_alternate`, `postal_address`) porte les contacts d'une bibliothèque. Le backend est **complet** : RLS `INSERT`/`UPDATE`/`SELECT` pour `authenticated`, RPC d'écriture `upsert_library_contact_profile` et RPC de lecture `get_library_contact_for_cooperation` toutes deux en place. Mais **aucun éditeur frontend** dans la page Biblioteca n'appelle `upsert_library_contact_profile` — la table est vide (0 ligne en prod). Conséquence concrète : `notify-document-permission-request` est structurellement muette faute de destinataire. Même motif qu'EA-08 (backend local-par-biblio prêt, éditeur absent). Révélé par le chantier #110 Resend (session R.3, 21/05). | Doctrine de complétude des parcours (un backend prêt sans interface = parcours inachevé) | **✅ Acté : écart Biblioteca de plein droit (backend local-par-biblio, pas réseau — distinct d'ER-01). À traiter par création d'un éditeur frontend appelant `upsert_library_contact_profile`, dans l'onglet `identity`. Chantier frontend pur, type EA-08. Rattachement de séquence : à arbitrer (étape dédiée ou greffe sur une étape touchant `identity`).** |

*Grille hybride : EA-00 cadre transverse + EA-01 à EA-20 par onglet ou transverse spécifique. **Total : 21 écarts** (19 issus de la cartographie + EA-19 révélé par le test fumée EA-15 + EA-20 révélé par le chantier #110 Resend). Phase 3 = arbitrage de chaque écart.*
...
| 8 | **EA-13 + EA-14 + EA-19** — câblage `sendReport` vers `notify-event` + envoi semanal automatique + mail d'invitation de tâche | 7-12h | dépendance #110 | **✅ LIVRÉ 08/06/2026** (#110 levé ; cf. §6.4) |
| 9 | **EA-12 phase 2** — parité fonctionnelle PEB (15-25h additionnelles, ~45 fn JS manquantes) | 15-25h | parité fonctionnelle | **DIFFÉRÉE 08/06 — gelée, conditionnée au retour prod BLMF↔BTL (cf. §6.4)** |
| 10 | **EA-11** — exchanges à parité HTML (~118 fn JS, refonte intégrale) | 40-60h | parité HTML radicale | **NON RETENUE 08/06 — React jugé fonctionnellement suffisant (cf. §6.4)** |
...
- 21 écarts identifiés : 19 issus de la cartographie initiale (1 cadre + 18 spécifiques/transverses) + EA-19 révélé par le test fumée EA-15 + EA-20 révélé par le chantier #110 Resend
- 10 écarts d'implémentation à traiter en sous-chantiers (entre 30 min et 60h chacun)
...
### 6.4 Clôture de l'étape 8 et du chantier-cadre (08/06/2026)

L'**étape 8** (EA-13 + EA-14 + EA-19), dernière réserve bloquante du chantier-cadre, est **livrée et vérifiée de bout en bout** le 08/06/2026, débloquée par la clôture de la migration #110 (Brevo → Resend) :

- **EA-13** — le bouton « Enviar relatório » du Painel ne fait plus de `mailto:` : il déclenche un envoi serveur réel via l'Edge Function `notify-weekly-report` (Resend), au moyen de la RPC `fn_send_weekly_report_now(p_library_id)` (`SECURITY DEFINER`, garde-fou `user_can_manage_library_notifications`, semaine ISO précédente, destinataire `weekly_report_email`). Vérifié sur 4 niveaux : pipeline vert (#1098), garde-fou actif (rejet `not_authorized` sans envoi), trace `pg_net` 200 + `ok:true`, mail reçu.
- **EA-14** — le digest hebdomadaire automatique est **actif** via le cron `anarbib-notify-weekly-report-weekly` (lundi 8h) appelant `fn_cron_notify_weekly_report_per_library`.
- **EA-19** — le mail d'invitation de tâche était **déjà livré** (`fn_task_invite` → trigger → outbox → `notify-internal-task`).

**Arbitrage sur les étapes 9-10 (acté le 08/06/2026, pour clore sans réserve dormante)** :

- **Étape 9 — EA-12 phase 2** (parité fonctionnelle PEB, ~45 fn JS) : **DIFFÉRÉE et gelée**, conditionnée au retour d'usage réel BLMF↔BTL en production. Ne se rouvre que si l'usage révèle un manque concret. Reclassée en ticket dormant au backlog, hors chantier-cadre.
- **Étape 10 — EA-11** (parité HTML radicale des échanges, refonte ~118 fn JS) : **NON RETENUE par défaut**. Le panneau React des échanges est jugé **fonctionnellement suffisant** ; répliquer l'exhaustivité de l'ancien HTML n'est pas un objectif du réseau. Réouvrable uniquement sur décision délibérée.

**En conséquence, le chantier-cadre Biblioteca est CLOS le 08/06/2026** sur sa mission : 12 onglets fonctionnels, parité doctrinale établie, transports mail câblés. Les gaps de parité HTML radicale (étapes 9-10) sont reclassés en suivi séparé dormant, non actifs. Décision normative actée au REGISTRE ; backlog v27 mis à jour (#BIBLIO clos + ticket dormant).
