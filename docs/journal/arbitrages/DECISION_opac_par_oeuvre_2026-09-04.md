...
6. **Les tomes vivent dans l'œuvre, jamais comme œuvre à part** (décision du soir, 04/09/2026). Un tome est un volume de l'œuvre-série ; le numéro se pose sur la notice (`books.volume`, jusqu'ici réservé aux périodiques dans le formulaire). L'assistant de doublons reçoit un onglet « Volumes » (`suggest_volume_groups` : même auteur, même titre sans marqueur de tome ; `group_books_as_volumes` pose les numéros et réunit dans une seule œuvre ; `dismiss_volume_group` écarte), le catalogage un champ « Tome / volume », l'OPAC trie les tomes et dit « N volumes » (migration `20260904170000`). Cent notices portaient un marqueur de tome dans leur titre le 04/09.
