...
| **Date de dernière mise à jour du registre** | 4 mai 2026 |
...
| **Sous-traitant** | Brevo SAS (anciennement Sendinblue), siège 7 rue de Madrid, 75008 Paris, France. Serveurs en UE. Voir section 5. |
| **Durée de conservation côté Brevo** | Selon les conditions de Brevo (logs des envois conservés ~30 jours). Aucun stockage durable côté Brevo. |
...
- **Chiffrement en transit** : HTTPS obligatoire sur toutes les communications client/serveur (Codeberg Pages, Supabase API, Brevo SMTP).
...
| **Brevo SAS** (ex-Sendinblue) | Envoi des mails transactionnels | UE (France/Allemagne) | DPA intégré aux CGU Brevo, accepté à l'inscription du compte | DPA accessible sur brevo.com/legal/privacypolicy. Éditeur français, conforme RGPD par construction. |
...
### 6.1 Situation actuelle (statut : transitoire)

L'instance Supabase d'AnarBib est actuellement hébergée dans la région **`sa-east-1` (São Paulo, Brésil)**, opérée techniquement par AWS.

Le Brésil ne bénéficie **pas** d'une décision d'adéquation de la Commission européenne (au sens de l'article 45 RGPD). Le transfert de données personnelles vers Supabase sa-east-1 est donc encadré par les **Clauses Contractuelles Types (CCT)** adoptées par la Commission européenne (décision **2021/914**, **module 2 controller-to-processor**), incluses dans le **DPA Supabase signé le 4 mai 2026** (référence document `TFXNN-HUMKJ-3WKP8-MZMYW`, version DPA datée du 5 août 2025).
...
- Supabase Inc. est soumise au RGPD via ses CCT et engage sa responsabilité européenne (loi applicable au DPA : Irlande).
- Aucun transfert nominal vers les États-Unis dans le cadre du fonctionnement de base. Les sous-processeurs ultérieurs US (Sentry, OpenAI éventuellement) sont identifiés et leur exposition est limitée par configuration (cf. §5.2).

### 6.2 Évolution prévue

Une migration vers une région européenne (`eu-west-1` ou `eu-central-1`) est **à l'étude**. La décision sera arbitrée avant l'élargissement du réseau au-delà de la bibliothèque pilote et de BLMF.

> **Pour les bibliothèques adhérentes** : si vous opérez votre propre instance Supabase, mentionnez ici **votre** région d'hébergement. Si elle est en UE, supprimez cette section. Si elle est hors UE, documentez les CCT applicables.
