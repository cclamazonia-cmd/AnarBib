...
**2026-08-29** · atualizado em **2026-09-24** · 69 itens · Version française : `AnarBib-Backlog-2026-08-29-v34.md`
...
**Frescor dos constatos em 2026-09-24.** **56 itens de 69** trazem uma verificação datada própria (A1, A3, B10, B13, B24, C3, C4, C7, C8, C9, C10, C11, D3, D6, E1, E2, E4, E6, E9, E14, E15, E19, E20, F1, F3, F4, F6, F7, F10, F11, F12, F13, F14, G1, G6, G8, G10, G13, G14, H2, H9, H10, H11, H13, I2, I3, I15, I18, I21, I24, I25, J2, J9, K2, K7, K10). Os **13** outros ainda repousam sobre o levantamento de 2026-08-29 e são assinalados como tais em cada ficha. Um constato não reverificado não é falso: é apenas velho, e a diferença vê-se aqui em vez de no uso. Esta linha é recalculada a cada geração do documento.
...
**Estado.** A PR #28 adiciona um transporte híbrido SMTP / Resend / « mock »: sem `SMTP_HOST` nem `RESEND_API_KEY`, `sendEmail` devolve `{ok:true, mocked:true}`. É o caso (a) de `DOC-SILENCE-1`. Em produção nada muda hoje; no dia em que o segredo faltar, a produção responderá 200 sem enviar nada. **D4 (21h30)**: no instalador, a simulação vira opção 3, nunca padrão. **23/09 — o transporte foi entregue (PR #30, merge `2cd27d71`).** O companheiro (`ASR2026`) retomou F7 depois de fechar a sua #29: `_shared/mail/smtp.ts` (cliente SMTP puro Deno) e o encaminhamento de `_shared/transport/email.ts` — `mock` **só** com `MAIL_TRANSPORT=mock`, SMTP com `SMTP_HOST` (salvo `MAIL_TRANSPORT=resend`), Resend por padrão, e um `throw` legível sem configuração. **O primeiro critério está cumprido.** Seis correções pedidas na releitura, todas feitas: RFC 2047 (codificar só o display-name — um nome de biblioteca acentuado quebrava o `From:`), STARTTLS obrigatório com opt-in explícito `SMTP_ALLOW_INSECURE`, corpo em base64 dobrado a 76 colunas (o que também neutraliza o dot-stuffing), guarda sobre `SMTP_HOST` ausente, tempos-limite `SMTP_TIMEOUT_MS`, e um banco `src/tests/smtp-transport.test.js` que carrega o módulo real. Suíte verificada sobre o resultado do merge: 75 arquivos, 791 testes. Depois do deploy, sonda OPTIONS das 10 funções que passam pelo transporte: 200/405 em todas, nenhum 503. **O item continua ABERTO no segundo critério**: oito funções ainda chamam a API Resend diretamente, **Retificação de 23/09**: `register` NÃO é silencioso — registra, devolve `email_usuaria_enviado: false` e a página de inscrição mostra um aviso dedicado. O caso (a) de `DOC-SILENCE-1` está noutro lugar: **`request-password-reset`** falha no envio mas o `catch` devolve 200 `{ok:true}` por anti-enumeração — decisão correta, que fica — de modo que uma chave rodada não aparece em lugar nenhum além de um `console.error`. Decidido em 23/09: a falha de transporte abrirá um incidente na cadeia de alerta. **Lote 1 da consolidação, entregue em 23/09**: `notify-weekly-report`, `notify-network-weekly-report` e `notify-rede-digest` passam por `_shared/transport/email.ts` com roteamento explícito (`noReplyTo` para o digest). Guarda `src/tests/mail-transport-routage.test.js`. **Lote 2, entregue em 24/09**: `notify-document-permission-request` e `notify-mid-loan-reading` passam pelo módulo compartilhado com roteamento explícito; o contrato local `{ok, status, body}` do segundo é preservado. **Lote 3, entregue em 24/09, e a lista fechada está VAZIA**: `notify-library-request`, `register` (contrato intacto; o módulo aprende `toEmails` e `transportConfigure()`) e `request-password-reset`. **Os dois critérios de F7 estão cumpridos.** O que resta é o caso (a) de `DOC-SILENCE-1` em `request-password-reset`: um incidente de supervisão a abrir, com um `kind` novo na CHECK e no `health-probe` juntos — obra à parte. **Comprovado em produção em 23/09 às 19h30**: uma reserva e o seu cancelamento dispararam quatro e-mails reais por `notify-event` (labels `user_mail` e `admin_copy`, recebidos dos dois lados), registrados como « [transport] envoi via Resend » — a maiúscula que só o código novo escreve; nenhum erro em seguida.
