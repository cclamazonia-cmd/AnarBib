...
**Frescor dos constatos em 2026-09-01.** **56 itens de 79** trazem uma verificação datada própria (A1, A3, B4, B7, B9, B10, B11, B13, B14, B17, C1, C2, C3, C4, C5, C7, C8, C9, C10, D1, D3, D6, E2, E5, E6, E7, E8, E9, F1, F3, F4, F6, F7, F8, G1, G2, G3, G4, G5, G6, G8, H1, H4, H5, I1, I3, I4, I6, I8, I10, I11, I12, I13, J2, J6, K2). Os **23** outros ainda repousam sobre o levantamento de 2026-08-29 e são assinalados como tais em cada ficha. Um constato não reverificado não é falso: é apenas velho, e a diferença vê-se aqui em vez de no uso. Esta linha é recalculada a cada geração do documento.
...
| Avisos de segurança | **505** | 0 ERROR · **453** + 28 WARN sobre funções DEFINER expostas · 24 INFO « RLS sem policy ». Os `anon` caíram de 36 para 28 (lote **B2**) e os `authenticated` de 464 para **453** ao longo dos sete pacotes do **B14** — cada queda é uma função fechada por uma razão escrita, nunca um ajuste |
...
| | | |
|---|---:|---|
| Pastas no repositório | **50** | + `_shared`; a 50ª é o roteador `main`, **nunca implantado no Supabase, e isso é intencional** — serve apenas à pilha auto-hospedada |
| Declarações `verify_jwt` | **38** | **todas em `false`**: as funções protegidas o são pelo padrão da plataforma, não por uma declaração. Reconciliar este arquivo com as funções realmente implantadas continua sendo o item **B6** |
...
| Testes | **349 + 61** | **349 casos JavaScript** em 25 arquivos — número dado pela execução, não por um grep das declarações. **61 suítes SQL, todas na CI** — a casa de máquinas, o arbítrio dos periódicos e os documentos digitais entraram na allowlist em 01/09 |
...
| | | | |
|---|---|---|---|
| **B14** | Auditar as funções `SECURITY DEFINER` abertas a `authenticated` | `P2` | Em curso |
...
#### B14 — Auditar as funções `SECURITY DEFINER` abertas a `authenticated`
...
*Verificado : 31/08 — relido à luz do lote 3 de **B2**, entregue no mesmo dia: o padrão de `public` traz agora `postgres`, `authenticated`, `service_role`. A contagem de 30/08 não foi refeita; **a auditoria em si continua inteira**. **01/09**: `pg_default_acl` relido — a entrada `FOR ROLE supabase_admin` ainda traz `EXECUTE` para `anon` e `authenticated`. Tratar as **duas** entradas, evitando duas vezes a armadilha da entrada esvaziada. **01/09 à noite — o lote `api` foi iniciado, e a primeira presa é um vazamento real.** As 138 recenseadas e triadas; a pilha « sem guarda visível » lida por inteiro: 23 delegam a guarda, e **uma só era verdadeira**: `get_due_date_for_loan` lia o estado de cotização de qualquer UUID — a forma exata de maio. Vazamento dormante (nenhuma biblioteca tem `membership_enabled`), corrigido antes da caixa de seleção: migração `20260831201011`, suíte de 4 testes, CI verde. Restam as 63 + 51 por pacotes, depois `public`. **01/09, pacote 2 — 15 funções nominativas lidas, um segundo vazamento dormente.** `get_member_restriction` guardava bem, mas seu bloco *global* lia `profiles` sem vínculo com a biblioteca. Limitado (`20260901074627`), **depois re-corrigido** (`20260901075511`): o limite estava dentro de um `IF`, deixando o `record` não atribuído — a função levantava em vez de calar. Pego pela suíte escrita no mesmo commit, reproduzido e verificado em produção. **39 de 138 lidas.** **01/09, pacote 3 — as listas: nenhum vazamento, mas seis recusas mudas.** Das 21 funções lidas, os escopos são respeitados. O achado é outro: seis funções — cinco relatórios de qualidade e `fn_authority_list` — recusavam com um `RETURN;` nu, tornando « sem direito » e « nada a ver » indistinguíveis. Num relatório de qualidade, uma lista vazia significa « o catálogo está são »: uma recusa disfarçada de balanço tranquilizador. Corrigido por substituição **verificada** (`20260901082124`), sem tocar nos direitos; guardado por uma suíte que verifica a **forma**. **60 de 138 lidas.** **01/09, pacote 4 — 45 escritas sobre objeto: nenhuma falha, e isso é um resultado.** Três pacotes seguidos tinham dado uma presa; este não dá nenhuma, na forma mais perigosa (agir sobre a coisa alheia). As 45 seguem uma doutrina implícita coerente, agora escrita: **a guarda segue a propriedade do objeto**. Três formas a imitar, entre elas `attach_exemplar`, que **não recebe a biblioteca como parâmetro** — deduz do chamador. **105 de 138 lidas, 33 restantes.** **01/09 — o lote `api` está ENCERRADO: 138 de 138 com veredito escrito**, fechamento verificado pelo segundo caminho (`DOC-RECENS-1`): zero não-lidas, zero nome fantasma. O pacote 5 não achou nada, e os dois atos mais pesados da rede são os mais bem guardados. **Balanço: dois vazamentos reais, seis silêncios, zero defeito nas 77 escritas e configurações** — ambos os vazamentos dormentes. A lição: *a pergunta útil não é « é explorável hoje? » mas « o que o armaria? »*. **01/09 — o esquema `public` foi iniciado (69 de 326), e a primeira presa corrige o meu próprio corretivo da manhã**: o helper `fn_is_loan_blocked_by_dues` continuava exposto atrás da fachada fechada. *Corrigir um caminho não corrige o que ele atravessava.* E um oráculo NÃO dormente: `fn_painel_find_profile_by_lookup` distinguia « existe mas não aqui » de « não existe » — teste de existência de e-mail numa rede militante. Quatro helpers internos fechados: 326 → 322 expostas. **01/09, pacote 2 de `public` — 19 nominativas lidas, e catorze irmãs do mesmo oráculo.** `fn_painel_get_profile_by_id` era a gêmea exata da função corrigida de manhã. O motivo revelou uma segunda (identificador **sequencial**), depois **doze mais** quando a suíte procurou a mensagem *sem os acentos*: toda a família `fn_import_*`. Corrigidas por substituição verificada, nenhum corpo recopiado. **Esta migração custou três vermelhos em `main`** ; três regras no REGISTRO (`DOC-MSG-1`). **88 de 326 lidas.** **01/09, pacote 3 de `public` — 57 listas com parâmetro de escopo: nenhum vazamento.** Cinco falsos positivos do meu detector, com a distinção escrita: *um `RETURN;` depois de um parâmetro vazio é legítimo, depois de uma recusa não*. **Uma questão de doutrina posta e não decidida**: quatro funções de rede guardam no `WHERE` — recusa muda, mas **deliberada e comentada**; deixada ao coletivo. **138 de 326 lidas, restam 188.** **01/09, correção da minha própria contagem**: eu havia escrito « 145 de 326 » somando os pacotes como se seus critérios se excluíssem — sete funções pertenciam a dois critérios e foram contadas duas vezes. O número real era **138**. *Um resto medido é uma medida; uma soma de pacotes é uma estimativa* — deduz-se o lido do resto, nunca o inverso. **01/09, pacote 4 de `public` — dois vazamentos, e a armadilha do remédio uniforme.** As 188 restantes foram retriadas com o **vocabulário real das guardas** (21 predicados extraídos por introspecção dos `IF NOT <chamada>`, seis deles ausentes das minhas listas feitas à mão): 39 sem guarda conhecida. `fn_next_tombo` não tinha nenhuma e devolve o próximo número de tombo de uma biblioteca — portanto **quantos exemplares ela já catalogou**; em laço pela rede, é a volumetria comparada dos acervos. `link_book_contributors_to_authors` também não tinha nenhuma **e ela escreve**. Mas a verificação de quem as chama, feita antes de escrever, mostra que **ambas são chamadas pelas telas de catalogação**: um `REVOKE` teria trocado uma recusa por uma tela morta — a armadilha de `confirm_pickup_v1`. Guarda no corpo para elas (`DOC-RPC-3`), revogação para as duas sem chamador em sessão. *A escolha entre guardar e revogar se lê em quem chama, nunca na função sozinha.* Testado em produção em transação revertida nos **dois** sentidos — leitor(a) recusade, equipe passando — depois o arquivo inteiro mais sua suíte, `B14_GARDES_ECRITURE OK : 4/4`, produção verificada intacta depois. **177 das 322 expostas lidas, restam 145.** **01/09, pacote 5 de `public` — o descompasso não estava nas funções.** Este pacote procurava, entre as escritas guardadas, a distância entre o objeto guardado e o objeto escrito (a falha exemplares/holdings de julho). As funções estão sãs, e todas na mesma forma: *a guarda se calcula a partir do objeto lido, nunca de um parâmetro* — `fn_record_deposit` e `fn_record_membership_payment` sequer aceitam biblioteca como parâmetro. O descompasso estava na visão que todas elas consultam. **`api.my_access` respondia a duas perguntas como se fossem uma só**: `can_access_painel` dizia « você tem papel de equipe EM ALGUM LUGAR? » e `library_id` dizia « qual é a sua biblioteca PRINCIPAL? ». **37 funções leem as duas juntas, 24 delas escrevem** (circulação, contribuições, depósitos, importação): quem é bibliotecárie em A e apenas leitor(a/e) em B, com B como principal, obtinha o painel de B. **Demonstrado em transação revertida**, armando o caso: a visão de então respondia `btl painel=true`, a corrigida responde `blmf painel=true role=librarian`. Uma pessoa está no caso hoje, salva pela ordenação; o que o armaria é *um gesto ordinário da interface* (marcar a outra biblioteca como principal). Correção na fonte, sem tocar nas 37: o vínculo efetivo prefere um papel de equipe, e `can_access_painel` se calcula sobre ele — **equivalente ao cálculo anterior, então ninguém perde acesso, por construção e não por sorte**. A forma a imitar estava no mesmo pacote: `resolve_managed_library_id` **reverifica** o direito sobre a biblioteca vinda de `my_access` em vez de confiar nela — *um valor vindo de uma visão de sessão é uma candidatura, não uma autorização*. **E minha própria triagem foi estreita demais**: 53 escritas listadas a partir de doze predicados adivinhados, 67 segundo o vocabulário obtido por introspecção — o defeito que o pacote 4 acabara de corrigir, refeito um pacote depois; o segundo caminho não se conquista de uma vez por todas. **Um desvio de doutrina medido e deixado para decidir**: `merge_book`/`merge_author` exigem a arbitragem (admin de rede ou coordenador(a/e)), enquanto `merge_serial` e suas duas irmãs aceitam qualquer `librarian` — os periódicos de 27/08 não retomaram a decisão do canteiro DUPLICATAS P4. **4 pessoas** são `librarian` sem ser `coordenador(a/e)`: podem destruir uma autoridade de revista, mas não uma de livro. Não corrigido — retirar um poder de quatro pessoas sem avisá-las não cabe num deploy automático. **01/09, as duas questões em aberto foram decididas no mesmo dia.** **(1) As guardas no `WHERE` passam a seguir `DOC-SILENCE-1`**: as cinco funções de rede agora levantam 42501 em vez de devolver lista vazia. A forma foi imposta pelo código — elas são `LANGUAGE sql`, sem bloco `IF` possível; convertê-las exigiria reescrever cinco corpos para mudar só uma guarda, o que `DOC-MSG-1` proíbe. Mantém-se o `WHERE` com um **predicado que levanta**, depois de verificar o único ponto que importa: *uma guarda que levanta dentro de um `WHERE` dispara quando a relação está VAZIA?* Sim (medido). O limite está escrito — depende do plano — e por isso a suíte **chama** as funções em vez de reler o corpo delas. **Armadilha simétrica evitada**: em `list_catalog_libraries` e `fn_team_list_invitations` o mesmo predicado AMPLIA um acesso, e uma substituição por padrão teria transformado a navegação comum em erro — correção nominativa, guardada nos dois sentidos. **Custo: um quarto vermelho, e uma regra ampliada.** Duas suítes atestavam o comportamento retirado (`mentions_orphelines` T10, `invitation_claims_lot2` T18). Eu havia aplicado `DOC-MSG-1` às **mensagens** e não ao **comportamento**: *passar de « devolve vazio » para « levanta » quebra tanto quanto o que atesta isso.* Corrigido após verificar que todas as outras chamadas rodam sob JWT de admin — para não emendar um segundo vermelho numa segunda suposição; o log da CI confirmou sem desvio. **(2) O arbítrio de periódicos passa a ser o mesmo dos livros, APÓS aviso prévio**: as três funções passam a `fn_is_dedup_arbiter()`. Catalogar e **sinalizar** seguem abertos à catalogação (DUPLICATAS P8) — encontrar uma duplicata e decidir sobre ela são atos diferentes. **A migração está escrita, testada e deliberadamente NÃO enviada**: ela retira um poder de quatro pessoas nomeadas, e o repositório já firmou a regra — mudar o que pertence a alguém sem avisar não cabe num deploy automático. Aviso redigido em pt-BR, a língua das quatro. **01/09, pacote 6 de `public` — o que trai nao e a palavra, e a ordem.** O pacote 2 havia fechado catorze oraculos de existencia procurando o MOTIVO da mensagem (« nao pertence », depois a variante sem acentos). Este pacote encontra **mais nove, e nenhuma diz « pertence »**: dizem « Acesso restrito », « Voce nao tem permissao », « Este emprestimo pertence a outra pessoa ». *Procurar um vocabulario so encontra o que fala a mesma lingua.* O criterio que as alcanca e **estrutural e mensuravel**: um teste de existencia colocado ANTES do controle de direito, sobre um identificador adivinhavel — levantado comparando a posicao das duas recusas em cada corpo, em toda a superficie exposta. Dezesseis funcoes nesse caso, **nove com identificador sequencial** (assets digitais, runs de importacao, conjuntos de regras de circulacao, exemplares, emprestimos). **Nao se inverte a ordem**: a guarda recai sobre a biblioteca DO OBJETO, que e preciso ter lido para conhecer — e essa e justamente a boa forma, a que o pacote 5 apontou como modelo. Unifica-se entao o que as duas recusas DIZEM, e o identificador sai da mensagem. **Uma correcao do pacote 2 estava pela metade**: `fn_attach_received_asset_record`, corrigida na mesma manha, havia mantido uma terceira mensagem de recusa — e uma so basta para reabrir o oraculo. A suite da manha passava em verde porque perguntava « o motivo desapareceu? »; a pergunta certa e « restam DUAS recusas distinguiveis? ». *Uma funcao corrigida nao e uma funcao encerrada.* **Um defeito da minha substituicao pego no ensaio em transacao revertida**: duas funcoes vizinhas partilham uma recusa sem partilhar o objeto (uma fala de « Asset », a outra de « Recurso digital »), e uma substituicao global fazia uma dizer a mensagem da outra. Substituicao dirigida por funcao. **Fora do escopo e declarado**: os identificadores `uuid` (um uuid nao se adivinha; corrigir seria ruido, e uma suite que fica vermelha em casos sadios deixa de ser lida) e as recusas de PAPEL que caem antes de qualquer leitura. `DOC-MSG-1` aplicado na forma ampliada da manha — as mensagens E ao comportamento: nenhuma asserção sobre os oito rotulos, nem nos testes nem no front. **01/09, pacote 7 de `public` — fechar o que ninguem chama.** Ultima classe, a mais sensivel numa rede militante: as leituras que devolvem dados NOMINATIVOS. Em 211 leituras expostas, 17 tocam `profiles`, 14 devolvem uma identidade — treze sao guardadas (entre elas `fn_list_membership_payments_for_user`, exemplar: exige o painel E filtra pela biblioteca ativa). **A decima quarta nao tem guarda alguma**: `fn_assembleia_facilitator_name` devolve nome e sobrenome para um identificador dado, e so responde se a pessoa for facilitadora — junta uma identidade a um PAPEL MILITANTE. O `CLAUDE.md` a cita entre os « vazamentos reais fechados » de 17/08: fora fechada para `anon`, e ninguem havia olhado `authenticated`. *Um vazamento fechado de um lado continua aberto do outro enquanto nao se nomear o lado* — e justamente o objeto do B14, e o seu melhor exemplo. **Segundo criterio: o que ninguem chama.** Uma RPC e chamada pelo FRONT, entao « sem chamador no banco » nada diz sozinho; cruzado com o repositorio, restam **53 funcoes expostas que nada chama em lugar nenhum**. *Esse criterio poderia ser falso e foi testado antes de ser aceito*: se um nome de RPC fosse construido por concatenacao, nenhuma busca textual o veria — verificado, nenhum e. **As 53 NAO sao revogadas**: muitas aguardam sua tela (todo o emprestimo entre bibliotecas, o percurso de candidatura), fecha-las quebraria um canteiro em vez de proteger. Lista levada a auditoria. **Cinco sao fechadas**, as que o proprio repositorio declara depreciadas ou substituidas e que tocam a identidade. **Uma ironia que vale licao**: as duas funcoes do painel fechadas hoje haviam recebido na mesma manha a correcao de oraculo do pacote 2 — *a correcao estava certa, o alvo nao*. Perguntar quem chama vale mais ANTES de escrever a migracao do que tres pacotes depois. **Uma guarda no WHERE permanece, e e motivada**: `fn_network_resolve_public_id` devolve NULL em vez de erro, e seu comentario o argumenta — *uma lista vazia afirma « nao ha nada », um valor nulo diz « sem resultado »*; a primeira e falsa para quem nao tem direito de ver, a segunda e verdadeira nos dois casos. A decisao da manha visava as listas que mentem; aplica-la aqui a transformaria em reflexo. **01/09, pacote 8 de `public` — agir ou ler por outrem: nada a corrigir.** Criterio: as funcoes que aceitam um IDENTIFICADOR DE PESSOA como parametro, a classe que dera a apreensao do lote `api`. **19 funcoes**, 11 delas escrevem; cinco sequer citam `auth.uid()`. **Duas sao lapides** — `fn_team_promote_to_coordenador` levanta `collegiality_required: direct promotion to coordenador is disabled` com um HINT que indica o percurso colegiado em tres tempos, e `fn_network_admin_request_removal` remete a votacao por unanimidade. Nao sao funcoes abandonadas, sao **decisoes de governanca materializadas no codigo**. *Uma funcao cujo corpo apenas levanta e um falso positivo estrutural de QUALQUER criterio « sem guarda »: ela nao tem o que guardar.* O detector deve exclui-las, senao grita a cada decisao bem tomada. **O modelo do esquema esta neste pacote**: `fn_painel_reader_other_memberships` devolve os outros vinculos de uma leitora — a informacao mais sensivel que uma rede militante pode deter — e so os nomeia sob **tres condicoes cumulativas**: parceria ativa, direito `transparence` explicitamente concedido, e **consentimento valido da pessoa**. Caso contrario a linha volta como NULL: a tela diz « ha outra coisa e voce nao tem direito de ver », o que e verdade, e deixa a pessoa decidir se conta mais. *O consentimento nao e uma caixa a mais alem da guarda, e um termo da guarda.* **Nenhuma falha nas 19** — um pacote sem correcao nao e um pacote sem resultado, e o que permite dizer que a classe foi passada. O lote `api` viveu o mesmo no seu pacote 5: sinal de que a superficie converge. **01/09, pacote 9 de `public` — a casa de maquinas.** Cartografia EM TRANSITIVO do que uma conta `authenticated` pode alcancar que toca a operacao : HTTP de saida (`pg_net`), segredos (`vault`), mecanica cron, filas de notificacao — a classe do primeiro vazamento do projeto (`fn_gazette_translate_call`). O exame direto nao bastava: `fn_send_weekly_report_now` le seus segredos por um wrapper que o padrao `vault\.` nao ve na chamadora — mesma licao do pacote 6, o criterio era o GRAFO de chamadas, nao o corpo. **Veredito: nenhuma falha, e e o melhor resultado possivel.** A arquitetura e em camadas e as camadas sao estanques: as ~30 funcoes motor (`fn_dispatch_*`, `fn_enqueue_*`, `fn_cron_*`, `fn_internal_get_vault_secret`, as chamadas LLM) estao TODAS fechadas a `authenticated`; as 44 funcoes de negocio expostas so as alcancam como DEFINER — o leitorado desencadeia consequencias, nunca a mecanica; duas excecoes fazem HTTP direto, cada uma guardada na sua biblioteca, URL e segredo vindos do vault e nunca de quem chama. `fn_gazette_translate_call` verificada fechada: o ciclo de 17/08 esta fechado dos dois lados. **Mas nada guardava essa estanqueidade** — um GRANT distraido em `fn_internal_get_vault_secret` teria dado TODOS os segredos do vault a qualquer conta da rede, sem alarme. Dai `salle_des_machines_tests.sql`, estrutural e nao nominativa: o criterio se remede a cada passagem e apanha as funcoes que ainda nao existem; so as duas excecoes HTTP sao nomeadas, e o T3 verifica que elas mantem sua guarda — *uma excecao sem guarda ja nao e excecao, e um buraco*. SALLE_DES_MACHINES OK 4/4. **E o arbitrio dos periodicos esta implantado**: aviso previo enviado pela coordenacao em 01/09 as quatro pessoas, migracao retida em local ate esse envio e entao enviada (reetiquetada `20260901145725`), suite `arbitrage_periodiques_tests` na CI. A regra — retirar um poder nunca parte sem que as pessoas tenham sido avisadas — foi cumprida na ordem que ela exigia. **01/09, pacote 10 de `public` — os documentos digitais : a cadeia e sadia, seu calcanhar e a arrumacao.** Auditoria de tudo que da acesso aos arquivos, policies de `storage.objects` incluidas (16 buckets). **Uma pista falsa instrutiva**: a policy de `pdf-restrito` so admite a equipe enquanto a RPC promete o acesso as leitoras `conta_ativa` — cheguei a crer a promessa quebrada. E a EF `read-digital-asset` que a sustenta, e a arquitetura merece registro: a RPC decide COM O JWT DE QUEM CHAMA, e a EF assina em `service_role` (TTL curto) — *a policy de storage nao e o porteiro do leitorado, a RPC e*. A EF ate mascara `source_url` de um documento restrito: a proveniencia e ela mesma uma informacao. **O EEB digital e um modelo**: `fn_ill_signed_url` revalida o direito de parceria NO MOMENTO do acesso — romper a parceria corta os recibos ja transmitidos (PARTNER-D5); *um direito que nao se revalida no acesso so e revogavel no nome*. E `digital_assets` so recebe recursos publicos: o dispositivo guarda o acesso ao servico, nao bytes secretos. **O calcanhar, que nada guardava: a arrumacao.** Um asset `conta_ativa` catalogado por engano num bucket publico seria servido pela URL publica — sem RPC nem EF, mundialmente — e NADA quebraria: o percurso normal continuaria funcionando. *O que torna a exposicao silenciosa e precisamente que nada quebra quando ela acontece.* Suite `documents_numeriques_tests` (4/4 em prod): nenhum asset nao-publico em bucket publico, os tres buckets restritos mantem `public=false` (uma virada = um clique de dashboard = tudo exposto), a RPC mantem suas duas condicoes — a EF assina tudo que ela devolve — e a revalidacao EEB permanece. **Duas observacoes sem correcao**: 4 objetos de `anarbib-media-restricted` inalcancaveis por quem quer que seja (residuo provavel), e a leitura direta de `pdf-restrito` aberta a equipe de qualquer biblioteca — coerente com a confianca de rede da catalogacao, mas e uma escolha, e agora esta escrita.*
...
- ~~As 138 funções de `api` têm um veredito escrito~~ — 138/138 em 01/09, fechamento verificado nos dois sentidos.
- ~~As funções a restringir perderam seu `EXECUTE` por migração~~ — tratadas de outra forma e melhor: nenhuma perdeu seu `EXECUTE` (a recusa vive no corpo), três migrações corrigiram as guardas faltosas.
- ~~O relatório vive em `docs/journal/audits/`~~ — `AUDIT_execute_authenticated_2026-09-01.md`, cinco pacotes datados.
- O resto de `public` segue — **145 de 326 lidas em 01/09**, restam 181. Um arbítrio pendente: as quatro guardas no `WHERE` (recusa muda deliberada) seguem `DOC-SILENCE-1` ou ganham uma exceção escrita ?
...
*Verificado : 31/08 — ainda 4 títulos e 7 fascículos no banco. A revisão da spec continua por fazer.*
...
*Verificado : 31/08 — `supabase secrets list` refeito: os mesmos treze segredos com a impressão da cadeia vazia. `REGIMENTO_URL` segue vazio e sem fallback.*
...
*Verificado : 31/08 — três `Resolve-DnsName`, mais o `send.` e o pai: SPF em `send.`, DKIM publicado, DMARC `p=none` com `rua` Brevo nos dois níveis, `brevo-code` residual.*
...
**Estado.** **Constato corrigido em 31/08: o circuito está sendo percorrido pela primeira vez — em real, não em ensaio.** `library_team_invitations` traz uma linha: convite `ebd78fb9` ao papel `coordenador` na **BTL**, proposto em 30/08, ratificado (status `ready`, e-mail enviado), ainda não aceito, expira em 29/09. Foi ele que produziu a linha `#72` de B17. O ensaio em seis passos em `blmf-teste` continua por fazer: uma promoção real está estreando o circuito em seu lugar.

*Verificado : 31/08 — medido no banco: a linha do convite, o evento enviado, e a linha `#72` de B17 partida no mesmo dia.*
...
*Verificado : 31/08 — nenhuma ocorrência de OPDS em `src/` nem `supabase/functions/`.*
...
*Verificado : 31/08 — o cron (`20 4 * * 2`, ativo) não tem nenhuma execução no registro. Primeira esperada em 01/09.*
