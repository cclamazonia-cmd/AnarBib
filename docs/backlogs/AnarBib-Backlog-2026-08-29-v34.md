...
| Fonctions applicatives | **917** | `public` 686 · `api` 188 · `ingest` 34 · `private` 9. Dont **704 SECURITY DEFINER** — +8 le 15/09 au soir : les sept de E21 (numérotation d'une bibliothèque, cotes et classes de rangement d'un lot) et `fn_gazette_submission_decision_enqueue` (GAZ-7, interne). Les 909 / 696 du relevé de 21 h sont ceux d'avant les deux livraisons de la soirée. |
| Migrations appliquées | **317** | 317 migrations numérotées au dépôt = **317 appliquées, alignement exact** (vérifié le 15/09 à 22 h 45, après les deux livraisons de la soirée : `20260915193525` GAZ-7 et `20260915201252` E21). 325 fichiers avec le gabarit et les 7 rollbacks. |
| Jobs `pg_cron` | **38** | actifs — +1 depuis le 03/09 (le tick de pré-traduction des titres d'œuvre, `work-titles-autofill`). |
| Avis de sécurité | **471** | 0 ERROR · **419** + **28** WARN sur les fonctions DEFINER exposées · 24 INFO « RLS sans policy » (la liste attendue de `bootstrap.sh`, verdicts B4). 419 = 411 + les huit RPC du 15/09 (bibliothèque de destination d'un lot, E21) — **toutes justifiées**, « Complément du 15/09 » de `AUDIT_execute_authenticated_2026-09-01` : aucune faille, la limite fonctionnelle 2 du 06/09 se referme (`fn_batch_caller_can_edit` rapproche le lot de la bibliothèque), une forme à corriger d'une ligne (trois RPC vérifient l'existence du lot avant les droits). Format groupé : compter `findings`, pas les entrées. |
| Avis de performance | **422** | **350 « index inutilisés »** (351 à 21 h, 368 le 06/09 — les compteurs repartent du 02/09 et baissent à mesure que les index servent), 38 FK sans index (toutes assumées, gardées par B21), 25 tables à policies permissives multiples, 8 tables sans clé primaire, 1 avis sur les connexions `auth`. Rien de neuf depuis la soirée. |
...
| Marqueurs de dette | **18** | dont 4 dans `src/` — **recompté à méthode fixe** (`git grep -E 'TODO|FIXME'` hors `docs/`) : 18 au commit `60e0580a` comme à `f7bf927c`, la soirée n'en a ajouté aucun ; le « 21 » du relevé précédent venait d'un balayage plus large. Aucun n'est une tâche ouverte : la dette nommée vit au backlog, pas dans le code. |
...
**État.** Mesuré le 07/09 en préparant la spec d'`I17` (§4 de `CADRAGE_rejeu_fidele_privileges_par_defaut_2026-09-07`). **Classe A** : 98 fonctions portent `anon=X` dans leur ACL ; pour **12** d'entre elles, aucun `GRANT … TO anon` n'existe dans le dépôt — elles ne sont ouvertes que par le privilège par défaut du moment de leur création (quatre DEFINER pourtant listées dans `T10`, dont `fn_book_restricted_pdf_state` et sa sœur appelées par le front ; six triggers et helpers des périodiques et des tomes ; `fn_current_user_is_member_of_holding_library`, `fn_reading_notes_enabled_for`). **Classe B** : 35 fonctions sont exécutables par `anon` via `PUBLIC` — `=X` explicite sur **dix RPC de circulation de `api`** (`advance_consulta`, `create_loan_at_counter`, `return_loan_total`…), 17 helpers et triggers d'`ingest`, 5 de `public` ; ACL nulle sur `api.extend_loan_item_as_library`, `api.renew_my_loan_item`, `private.fn_book_work_id`. Toutes INVOKER : la RLS tient, le lint 0028 ne les voit pas — mais une RPC de prêt appelable par un visiteur anonyme est une surface laissée par oubli, pas par décision (`DOC-GRANT-1`). **07/09, expérience d'`I17`** : même mécanisme côté **relations**. Cinq vues du socle sans `security_invoker` (`v_author_alias_candidates_unique`, `v_author_alias_worklist`, `v_author_seed_candidates`, `v_terra_livre_books_ready`, `v_terra_livre_books_ready_stats`) naissent lisibles par `anon`/`authenticated` au rejeu (défaut de l'image sur les tables : `anon=arwdm`) alors que la prod ne leur laisse que `anon=m` : le `REVOKE SELECT` qui les a fermées n'est écrit nulle part (`20260830110000` ne retire qu'`insert, update, delete`). T7 de `grants_herites` est rouge au rejeu, vert en CI (template0). Deux voies : étendre A.1 aux tables avant le socle, ou une migration nominative — vues et policies à chercher avant. **16/09 : livré, à mesurer en prod.** Relevé du jour (lecture seule) : 133 fonctions exécutables par `anon` sur les quatre schémas, 47 sans `GRANT … TO anon/PUBLIC` écrit (classement du dépôt par script : 12 à `anon=X` par défaut de création, 35 par `PUBLIC` dont 3 à ACL nulle) — le même compte que le 07/09. Appelants cherchés avant tout REVOKE (vues avec leur security_invoker, policies, prosrc + prosecdef, triggers, expressions d'index, défauts générés) : quatre ouvertures SERVENT sous anon et n'étaient écrites nulle part — `private.fn_book_work_id` (lue par les vues invoker `api.catalog_list_anon_v1`/`_session_v1`), les deux `fn_book_restricted_pdf_state*` (page publique du livre, EF read-pdf), `fn_volume_rank` (appelée par `api.catalog_works_v1` et `work_public_detail`, INVOKER) ; les 43 autres se ferment (10 RPC de circulation + 2 à ACL nulle : le front ne les appelle que connecté ; 17 helpers/triggers d'ingest, `authenticated` gardé pour les 3 lues par les vues invoker de l'écran d'import et les 2 en expression d'index sur books/book_drafts ; 5 triggers/helpers des périodiques + `fn_serial_issue_key` (défaut généré, `authenticated` gardé) ; `fn_title_sans_volume`, `fn_title_lisible_sans_volume`, `fn_conv_est_non_agent`, 3 `fn_assert_*` ; et deux DEFINER de T10 qui changent de camp, `fn_current_user_is_member_of_holding_library` et `fn_reading_notes_enabled_for` : 28 → 26). Les cinq vues de T7 : `REVOKE SELECT` écrit (personne ne les lit sous anon/authenticated : `api.report_auteurs_non_resolus` est DEFINER, aucune policy). Migration `20260916223000_b22_ouvertures_a_anon_ecrites` (bloc DO de contrôle : 4 ouvertes, 43 fermées, 5 vues) ; `grants_herites_tests.sql` : T10 à 26 entrées, **T12 = liste fermée des 90 fonctions exécutables par anon** (public, api, ingest, private, INVOKER compris), vérifiée dans les deux sens. Banc CI complet vert (103 suites, 12/12), rejeu sur image vert (321/321, 90 anon, MD5 `1852f6b2…`). Reste : la CI déploie, puis relever en prod le lint 0028 (attendu 26, plus 28) et le compte 90 — et clore.

*Vérifié : 16/09 — production lue (133 fonctions, ACL, appelants par pg_views/pg_policy/pg_proc/pg_trigger/pg_index/pg_attrdef), dépôt classé par script, banc CI complet et rejeu sur image `17.6.1.084` verts (worktree `claude/b22`).*

**Ce que c'est.** Après le déploiement : `get_advisors` doit rendre 26 pour 0028 (plus 28), et la production 90 fonctions exécutables par `anon` avec l'empreinte du rejeu. Puis clore. Pour toute fonction future : GRANT écrit dans la migration qui la crée ET ligne dans T12, sinon T12 rougit.
...
*Renvois : `journal/cadrages/CADRAGE_rejeu_fidele_privileges_par_defaut_2026-09-07 §4` · `REGISTRE §0 DOC-GRANT-1` · `tests/sql/grants_herites_tests.sql T10` · `item I17` · `supabase/migrations/20260916223000_b22_ouvertures_a_anon_ecrites.sql` · `tests/sql/grants_herites_tests.sql`*
...
*Vérifié : 06/09 — PR #28 relue en entier (46 fichiers, tête `b5782ec1`), production interrogée en lecture seule, constat `CONSTAT_PR28_rejeu_vs_production_revoke_anon_2026-09-06`. **15/09** — quatre têtes re-mesurées par l'API ; `install.sh` exécuté quatre fois sur pile isolée (`COMPOSE_PROJECT_NAME=anarbib-pr28`, worktree détaché sur `refs/pull/28/head`, stdin fermé) ; comptes `anon`/DEFINER et empreinte MD5 comparés à la production en lecture seule ; `merge-tree` contre `main` avant fusion ; CI du commit de fusion suivie jusqu'au vert.*
...
*Vérifié : 15/09 — journal `journalctl --user -u anarbib-backup-storage.service` : `Main process exited, code=killed, status=15/TERM` à 08:23:45, « Failed to enqueue OnFailure » ; `fn_backup_heartbeat_status` : `storage` `interrompu = true`, `muet = true`, 215,5 h. Relance manuelle à 20 h 50, en cours au moment du relevé. **21 h 05** — la relance manuelle a abouti : seize buckets resynchronisés, instantané `daafc96b`, témoin de vie envoyé, rétention 7/4/6 appliquée (l’instantané du 09/08 sorti). Le flux `storage` est de nouveau attesté ; le trou va du 06/09 21 h 19 au 15/09 21 h 05.*
