...
**2026-08-29** · mis à jour le **2026-09-24** · 69 items · Versão em português : `AnarBib-Backlog-2026-08-29-v34.pt-BR.md`
...
**Relevé du 22 septembre 2026 au soir** (`60b16828`) — production interrogée en lecture seule (`execute_sql`, advisors, journaux) et dépôt recompté ; **toutes les lignes ont été remesurées**, comme la veille au soir (`2a9681e5`, relevé complet, 56 commits en un jour). Ce qui a bougé en un jour : +2 migrations (330 = 330), +17 commits, +4 bancs, +1 clé, +1 suite SQL, 52 « index inutilisés » de moins ; fonctions, avis de sécurité et marqueurs de dette inchangés ; `auth.users` porte 20 comptes vivants et 2 jetons pseudonymes de comptes effacés. **Clos ce soir sur pièces : B27** — zéro `57014` sur `catalog_works_v1` en 24 h de journaux, 337 appels tous en 200, sonde en place (voir la clôture pour la nuance : ce trafic est celui de la sonde). **Clos sur pièces, avec leur mesure** : **I26** (une instance restaurée retrouve ses 38 crons — `private.fn_crons_replanifier()`, aller-retour réel sur `pg_cron`, sans effet en production), **E22** (l'accolade orpheline de `PanelPage.css` et la garde qui compte les accolades), **I27** (le chemin de secours du front a publié pour de vrai sur `app.anarbib.is`, preuve par `/.version-front`). Livré sans item : la sonde `images_pins` de `health-probe` et ses deux remesures (GoTrue v2.197.0, Storage v1.77.0 — la production a monté deux fois dans la journée), l'historique des migrations restauré avec le dump (troisième fichier), `deploy.sh` qui reconstruit enfin le front, `publier-front.sh`. **Ce qui reste à clore, et par qui** — *(a) par Xavier, sans code* : **F11** — ouvrir un courriel de chaque famille dans un vrai client et dire s'il se lit ; le code est en production depuis le 22/09 (`MAIL-Q7`), la garde tient, seul le regard manque. **I24** — requalifier la fiche : la sonde serveur alerte bien (deux courriels reçus), le gestionnaire systemd du poste sort sans cause établie ; décider si l'item vit encore. *(b) par une session, avec des mesures* : **I21** — sept conditions sur huit restent à cocher avec preuve (la connexion locale l'est depuis le 26/08, refaite le 21/09) ; **I2** — proxy inverse avec tunnel, tags en empreintes `sha256`, répétition à froid un mois plus tard, la question du Jitsi à poser à l'hébergeur. *(c) à surveiller, rien à faire* : l'incident `images_pins` se rouvrira de lui-même à la prochaine montée de GoTrue ou Storage — le courriel dit alors quoi faire (`deploy/banc-paliers.sh`, puis une migration qui remplace l'attendu, `.env.example` et `bootstrap.sh` ensemble). Pièce : `journal/operations/NOTE_pins-images-remesures_2026-09-21`.
...
**Fraîcheur des constats au 2026-09-24.** **56 items sur 69** portent une vérification datée qui leur est propre (A1, A3, B10, B13, B24, C3, C4, C7, C8, C9, C10, C11, D3, D6, E1, E2, E4, E6, E9, E14, E15, E19, E20, F1, F3, F4, F6, F7, F10, F11, F12, F13, F14, G1, G6, G8, G10, G13, G14, H2, H9, H10, H11, H13, I2, I3, I15, I18, I21, I24, I25, J2, J9, K2, K7, K10). Les **13** autres reposent encore sur le relevé du 2026-08-29 et sont signalés comme tels sous chaque fiche. Un constat non revérifié n'est pas faux : il est seulement vieux, et la différence se voit ici plutôt qu'à l'usage. Cette ligne est recalculée à chaque engendrement du document.
...
| Fonctions applicatives | **924** | `public` 688 · `api` 189 · `ingest` 34 · `private` 13. Dont **708 SECURITY DEFINER**. **Inchangé depuis le 21/09 au soir** : aucune fonction créée ni supprimée (comptage par schéma identique, aucun `oid` postérieur aux cinq du 21/09) — les deux migrations du 22/09 remplacent des corps existants. Le lint 0029 ne bouge donc pas. |
| Migrations appliquées | **330** | **330 appliquées en production = 330 numérotées au dépôt**. Les deux du 22/09 : `20260922185953` (délai de grâce de 15 min sur les files de notifications, sonde) et `20260922214500` (B28 : l effacement d un compte re-pointe sur le jeton pseudonyme les 26 colonnes de FK que rien ne traitait). Dernière appliquée : `20260922214500` ; rien n attend en file. |
| Jobs `pg_cron` | **38** | actifs — +1 depuis le 03/09 (le tick de pré-traduction des titres d'œuvre, `work-titles-autofill`). |
| Avis de sécurité | **470** | 0 ERROR · **420** WARN sur les DEFINER exposées à `authenticated` (0029), tous justifiés · **26** sur celles exposées à `anon` (0028), exactement la liste nommée T10 de `grants_herites_tests.sql` · 24 INFO « RLS sans policy » (la liste attendue de `bootstrap.sh`, verdicts B4). **Inchangé depuis le 20/09**, relu le 22/09 au soir. Format groupé : compter `findings`, pas les entrées. |
| Avis de performance | **363** | **291 « index inutilisés »** (343 le 21/09, 344 le 20/09, 368 le 06/09 — 52 de moins en un jour : les compteurs repartent du redémarrage du 02/09 et baissent à mesure que les index servent ; la cause n est pas relevée index par index), 38 FK sans index (toutes assumées, gardées par B21), 25 tables à policies permissives multiples, 8 tables sans clé primaire, 1 avis sur les connexions `auth`. |
...
| Comptes | **22** | **24** appartenances actives (inchangé) ; **22 lignes** dans `auth.users` : **20 comptes vivants** (fournisseur `email`, confirmés, chacun avec profil et appartenance) et **2 jetons pseudonymes** de comptes effacés (fournisseur `pseudonymized`, sans date de création, sans appartenance, jamais connectés — c est la trace que laisse `fn_delete_my_account`). La ligne disait 20 : elle comptait les vivants, ou les jetons n existaient pas encore — le relevé du 16/09 ne le dit pas. Quand ces deux effacements ont eu lieu n est pas relevé (les jetons ne portent pas de date). |
...
| | | |
|---|---:|---|
| Commits | **2 802** | +17 depuis le relevé du 21/09 au soir, dont **14 le 22/09**, tous de sessions voisines : **B28** (26 colonnes de FK que l effacement d un compte ne re-pointait pas — migration et suite vivante sur `pg_constraint`), **F14** (plus aucun `Reply-To` étranger au domaine d expédition), **F11** (tout fond de courriel porte sa couleur de texte), **E22** (l accolade orpheline de `PanelPage.css` et sa garde), la suppression du compte contributeur depuis sa page et le mot de passe refusé qui dit pourquoi, le délai de grâce de la sonde des files, l essai de bascule authentifié (OPS-10, runbook et registre), **I27** clos. Tête au moment du relevé : `60b16828`. |
| Fichiers `src/` | **356** | +4 le 22/09, quatre bancs d essai : `css-accolades-equilibrees` (E22), `mails-fond-et-couleur` (F11), `mot-de-passe-refuse-dit-pourquoi` et `reply-to-meme-domaine` (F14). Aucun fichier de page ni de composant ajouté. |
...
| Tests | **783 + 108** | **783 tests JS** (vitest, gate bloquant, 74 fichiers — relancés en entier le 22/09 à 21 h 52 sur `60b16828`, tous verts, deux passes) + **108 suites SQL** dans `ci-suites.txt` (+1 : `effacement_compte_fk_tests`, B28, dont le premier cas relit `pg_constraint` : une FK nouvelle non traitée par `fn_delete_my_account` fait rougir la CI). +86 tests JS en un jour : les quatre bancs du 22/09 et des cas ajoutés à des bancs existants. |
| Marqueurs de dette | **18** | dont 4 dans `src/` — méthode fixe (`git grep -E 'TODO|FIXME'` hors `docs/`) : 18 au 22/09 comme aux 21, 20, 16 et 15/09. Aucun n est une tâche ouverte : la dette nommée vit au backlog, pas dans le code. |
...
**État.** La PR #28 ajoute un transport hybride SMTP / Resend / « mock » dans `_shared/transport/email.ts` et dans les deux copies locales de `notify-library-request` et `notify-document-permission-request` : sans `SMTP_HOST` ni `RESEND_API_KEY`, `sendEmail` rend `{ok:true, mocked:true}` et journalise. C'est le cas *(a)* de `DOC-SILENCE-1`, et la forme exacte de son occurrence (1). En prod la clé Resend existe, rien ne change aujourd'hui ; le jour où le secret manque, la prod répondra 200 en n'envoyant rien. L'aiguillage SMTP/Resend, lui, est correct (`SMTP_HOST` posé ⇒ SMTP sauf `MAIL_TRANSPORT=resend`), et le client SMTP maison (`smtp.ts`) est propre. **D4 (21 h 30)** : côté installateur, la simulation devient l'option 3, jamais par défaut. **23/09 — le transport est livré (PR #30, fusion `2cd27d71`).** Le camarade (`ASR2026`) a repris F7 après avoir fermé sa #29 : `_shared/mail/smtp.ts` (client SMTP pur Deno) et l'aiguillage de `_shared/transport/email.ts` — `mock` **uniquement** sur `MAIL_TRANSPORT=mock`, SMTP sur `SMTP_HOST` (sauf `MAIL_TRANSPORT=resend`), Resend par défaut, et un `throw` lisible quand rien n'est configuré. **Le premier critère est tenu.** Six corrections demandées en relecture, toutes obtenues : RFC 2047 (n'encoder que le display-name — un nom de bibliothèque accentué cassait le `From:`), STARTTLS obligatoire avec opt-in explicite `SMTP_ALLOW_INSECURE`, corps en base64 plié à 76 colonnes (ce qui neutralise aussi le dot-stuffing), garde sur `SMTP_HOST` absent, délais d'attente `SMTP_TIMEOUT_MS`, et un banc `src/tests/smtp-transport.test.js` qui charge le vrai module par esbuild. Suite éprouvée sur le résultat de la fusion : 75 fichiers, 791 tests. Après déploiement, sonde OPTIONS des 10 fonctions qui passent par le transport : 200/405 partout, aucun 503. **L'item reste OUVERT sur son second critère** : huit fonctions appellent encore l'API Resend en direct (`notify-rede-digest`, `notify-library-request`, `notify-network-weekly-report`, `notify-document-permission-request`, `notify-weekly-report`, `notify-mid-loan-reading`, `register`, `request-password-reset`), **Rectification du 23/09** : `register` n'est PAS silencieux — il journalise, renvoie `email_usuaria_enviado: false` dans sa réponse, et la page d'inscription affiche un avertissement dédié (`auth.create.successEmailFailed`). La phrase précédente, écrite ici le matin même, décrivait le mécanisme sans regarder ce qui en est fait. Le cas *(a)* de `DOC-SILENCE-1` est ailleurs : **`request-password-reset`** lève bien à l'envoi, mais son `catch` rend 200 `{ok:true}` par anti-énumération — décision juste, qui doit rester — si bien qu'une clé tournée ne se voit ni côté demandeur ni nulle part ailleurs qu'un `console.error`. Décidé le 23/09 : l'échec de transport y ouvrira un incident dans la chaîne d'alerte. **Lot 1 de la consolidation, livré le 23/09** : `notify-weekly-report`, `notify-network-weekly-report` et `notify-rede-digest` passent par `_shared/transport/email.ts`, avec un routage explicite en argument (`noReplyTo` pour le digest, qui n'a jamais porté de `Reply-To`) — le module partagé en posait un d'office, c'était le piège. Garde `src/tests/mail-transport-routage.test.js` : payload figé et liste fermée des fonctions encore en direct, éprouvée par mutation. **Lot 2, livré le 24/09** : `notify-document-permission-request` (expéditeur d'environnement, `reply_to` seulement si `REPLY_TO_EMAIL` est valide — vide en production) et `notify-mid-loan-reading` (routage par contexte de bibliothèque ; son contrat local `{ok, status, body}` sans `throw` est préservé en traduisant l'exception du module, statut HTTP conservé quand le message le porte). Le balayage de F14 (`reply-to-meme-domaine.test.js`) voit désormais aussi un Reply-To passé en routage explicite, pour ne pas perdre de vue une valeur d'environnement en la déplaçant. **Lot 3, livré le 24/09, et la liste fermée est VIDE** : `notify-library-request` (en `noReplyTo`, F14), `register` (contrat `{requested, apiAccepted, status, responseText}` intact ; le module partagé apprend `toEmails` — une liste de destinataires en un seul message — et `transportConfigure()`, parce que la garde d'environnement exigeait `RESEND_API_KEY` et aurait rendu `MISSING_ENV` à toute inscription sur une pile SMTP) et `request-password-reset`. **Les deux critères de F7 sont tenus** : sans `MAIL_TRANSPORT=mock` explicite une fonction sans service lève un message lisible, et une seule implémentation d'envoi vit dans `_shared/transport/email.ts`, appelée par toutes. Ce qui reste n'est plus F7 mais le cas (a) de `DOC-SILENCE-1` dans `request-password-reset` (le `catch` anti-énumération avale l'échec de transport) : décision du 23/09 d'y ouvrir un incident de supervision — un `kind` à ajouter à la CHECK de `service_health_incidents` ET à `health-probe` ensemble, avec une sonde structurelle qui lit les échecs récents ; chantier à instruire à part. **Éprouvé en production le 23/09 à 19 h 30** : une réservation puis son annulation ont fait partir quatre courriels réels par `notify-event` (labels `user_mail` et `admin_copy`, reçus côté lecteur·rice et côté bibliothèque), journalisés « [transport] envoi via Resend » — la majuscule que seul le code fusionné écrit, l'ancien journalisait « resend » ; aucune erreur dans la foulée.
...
Backlog v34, écrit le 2026-08-29, mis à jour le 2026-09-24. Remplace `AnarBib-Backlog-2026-06-17-v33.md`. 69 items sur 11 domaines. L'état chiffré a été relevé le 2026-09-22 contre la base de production en lecture seule et contre le dépôt Codeberg au commit `60b16828` ; les items retouchés depuis portent leur propre date dans leur texte. Ce document n'arbitre rien : le `REGISTRE_decisions.md` fait foi.
