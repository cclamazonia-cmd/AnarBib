...
| **B28** | `fn_delete_my_account` échoue pour qui a agi — 26 colonnes de FK en NO ACTION / RESTRICT non re-pointées, dont les propositions d'autorité | `P2` | En cours |
...
#### B28 — `fn_delete_my_account` échoue pour qui a agi — 26 colonnes de FK en NO ACTION / RESTRICT non re-pointées, dont les propositions d'autorité

`P2` Courant · État : **En cours** · Charge : une soirée · Ce que ça demande : SQL / PostgreSQL

**État.** **Trouvé le 22/09/2026** en câblant la suppression de compte sur la page contributeur (même RPC que la page lecteur). **Rectifié le soir même** : ma première lecture de `pg_constraint.confdeltype` inversait les codes — `authority_duplicate_reports` et `library_profile_proposals`, que l'item nommait, sont en SET NULL et ne bloquent rien. Le relevé complet en production (`confdeltype IN ('a','r')`, hors schéma `auth`, colonne non citée dans le corps de la fonction) donne **26 colonnes** dans 20 tables : `authority_proposals.proposed_by` et `authority_proposal_objections.objecting_by` (NO ACTION — **tout·e contributeur·ice qui a proposé une chose ne pouvait pas supprimer son compte**), `library_requests.submitted_by_user_id` (RESTRICT), `library_request_messages.author_id`, `publishers.created_by`/`updated_by`, `cartography_entries.updated_by`, `cartography_submissions.reviewed_by`, `gazette_issue_locales.updated_by`, `gazette_submissions.reviewed_by`, `lettre_issues.created_by`/`sent_by`, `library_opening_hours.updated_by`, `library_public_contact.updated_by`, `library_request_claims.revoked_by_user_id`, `library_request_comments.author_admin_id`, `library_request_invitations.initiated_by`, `library_requests.proposed_by_admin_id`, `library_retention_policies.updated_by`, `library_team_invitation_ratifications.ratifier_user_id`, `library_team_invitations.proposed_by`, `library_unarchive_log.unarchived_by`, `reader_library_messages.deleted_by`/`staff_archived_by`, `user_library_memberships.pending_removal_requested_by`/`physically_validated_by_user_id`. Chacune refuse le `DELETE FROM profiles` par une violation de FK, message brut à l'écran.

*Vérifié : 22/09 — `pg_constraint` en production, filtre `conrelid ~ 'authority|contributor|proposal'` (relevé partiel : les autres tables à FK NO ACTION vers profiles restent à lister, c'est l'objet de la garde) ; définition réelle lue par `pg_get_functiondef`. 22/09 soir — relevé complet `pg_constraint` en prod (26 colonnes, décodage vérifié : a=NO ACTION, r=RESTRICT, n=SET NULL, c=CASCADE) ; définition réelle relue ; migration et suite écrites.*

**Ce que c'est.** Migration `20260922214500_b28_…` : repartie de la définition réelle (`pg_get_functiondef`, 22/09), un bloc « ACTES NOMMÉS » re-pointe les 26 colonnes sur le jeton pseudonyme (`v_token`), comme la gouvernance, et la réponse compte `pseudonymized_act_rows`. Suite `tests/sql/effacement_compte_fk_tests.sql` : T1 relit `pg_constraint` (liste VIVANTE : une FK nouvelle non citée fait rougir), T2–T5 effacent un compte qui a créé un éditeur et déposé une demande d'entrée. Clore quand la CI est verte et la migration en production.
...
- La suite est verte en CI : aucune FK NO ACTION/RESTRICT vers profiles/auth.users hors de la fonction, et un compte qui a agi se supprime.
- La migration est appliquée en production (`supabase_migrations`), et la suppression d'un compte contributeur ayant une proposition passe.
