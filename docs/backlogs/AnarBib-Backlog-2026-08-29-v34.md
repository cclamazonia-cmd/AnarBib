...
**2026-08-29** · mis à jour le **2026-09-01** · 79 items · Versão em português : `AnarBib-Backlog-2026-08-29-v34.pt-BR.md`
...
**Fraîcheur des constats au 2026-09-01.** **56 items sur 79** portent une vérification datée qui leur est propre (A1, A3, B4, B7, B9, B10, B11, B13, B14, B17, C1, C2, C3, C4, C5, C7, C8, C9, C10, D1, D3, D6, E2, E5, E6, E7, E8, E9, F1, F3, F4, F6, F7, F8, G1, G2, G3, G4, G5, G6, G8, H1, H4, H5, I1, I3, I4, I6, I8, I10, I11, I12, I13, J2, J6, K2). Les **23** autres reposent encore sur le relevé du 2026-08-29 et sont signalés comme tels sous chaque fiche. Un constat non revérifié n'est pas faux : il est seulement vieux, et la différence se voit ici plutôt qu'à l'usage. Cette ligne est recalculée à chaque engendrement du document.
...
| Migrations appliquées | **257** | 257 fichiers au dépôt : **tout est déployé**, y compris l'arbitrage des périodiques — retenu en local jusqu'à l'envoi du préavis aux quatre personnes concernées, poussé le 01/09 une fois le préavis parti |
...
| Avis de sécurité | **505** | 0 ERROR · **453** + 28 WARN sur les fonctions DEFINER exposées · 24 INFO « RLS sans policy ». Les `anon` sont passés de 36 à 28 (lot **B2**) et les `authenticated` de 464 à **453** au fil des sept paquets de **B14** — chaque baisse est une fonction fermée pour une raison écrite, jamais un réglage |
...
| | | |
|---|---:|---|
| Commits | **2 417** | 152 depuis le relevé du 29/08 — l'essentiel est la campagne de vérification du backlog et les sept paquets de l'audit **B14** |
| Fichiers `src/` | **290** | 79 pages, 89 composants |
| Clés i18n | **6 179** | × 10 locales, parité stricte vérifiée en CI |
| Tests | **349 + 61** | **349 cas JavaScript** dans 25 fichiers — chiffre donné par l'exécution, non par un grep des déclarations. **61 suites SQL, toutes en CI** — la salle des machines, l'arbitrage des périodiques et les documents numériques ont rejoint l'allowlist le 01/09 |
...
| | | | |
|---|---|---|---|
| **B14** | Auditer les fonctions `SECURITY DEFINER` ouvertes à `authenticated` | `P2` | En cours |
...
#### B14 — Auditer les fonctions `SECURITY DEFINER` ouvertes à `authenticated`
...
*Vérifié : 31/08 — relu à la lumière du lot 3 de **B2**, livré le jour même : le défaut de `public` sur les fonctions porte désormais `postgres`, `authenticated`, `service_role` — `anon` retiré, la ligne `pg_default_acl` vérifiée présente après coup. Le comptage du 30/08 (138 de `api` sur 142, 326 de `public` sur 498) n'a pas été refait ; **l'audit lui-même reste entier**. **01/09** : `pg_default_acl` relu — l'entrée `FOR ROLE supabase_admin` porte toujours `EXECUTE` pour `anon` et `authenticated`. Le retournement de B2 ne couvre que ce que `postgres` crée : traiter les **deux** entrées, et éviter deux fois le piège de l'entrée vidée. **01/09 au soir — le lot `api` est entamé, et la première prise est une fuite réelle.** Les 138 recensées et triées (51 à prédicat délégué, 63 à `auth.uid()` direct, 24 sans garde visible) ; la pile « sans garde » passée en entier : 23 délèguent leur garde (le regex ne voyait pas `resolve_managed_library_id`, `user_can_manage_library`, `fn_constitution_guard` — DOC-RECENS-1 appliqué au tri lui-même), et **une seule était vraie** : `get_due_date_for_loan` lisait l'état de cotisation de n'importe quel UUID — la forme exacte de mai, un identifiant en paramètre, une donnée nominative en retour. La garde existait dans la fonction *suivante* ; le bloc fautif s'exécutait avant. **Fuite dormante** (aucune biblio n'a `membership_enabled`), corrigée avant la case à cocher : migration `20260831201011`, suite `b14_api_cotisation_autrui_tests` (4 tests, l'effet et non le code), CI verte, vérifiée en prod. Audit : `AUDIT_execute_authenticated_2026-09-01.md`. Restent les 63 + 51 à lire par paquets, puis `public`. **01/09, paquet 2 — 15 fonctions nominatives lues, une seconde fuite dormante.** `get_member_restriction` gardait bien (staff de `p_library_id`) mais son bloc *global* lisait `profiles WHERE id = p_user_id` sans lien avec la bibliothèque : un·e staff de n'importe où lisait le gel réseau de n'importe quel UUID — raison, date, auteur·rice. Zéro compte gelé au 01/09, donc dormante comme la première. Bornée (`20260901074627`), **puis re-corrigée le quart d'heure suivant** (`20260901075511`) : le bornage était enfermé dans un `IF`, laissant le `record` non assigné sur le chemin « étrangère » — la fonction levait au lieu de se taire. Attrapé par la suite écrite dans le même commit (T1 rouge, T2/T3 verts), reproduit puis vérifié corrigé **en production sur données réelles**. Les 13 autres sont propres, dont deux exemplaires citées à l'audit. **39 des 138 lues.** **01/09, paquet 3 — les listes : aucune fuite, mais six refus muets.** Sur 21 fonctions lues au critère « une liste à partir d'un paramètre de portée », les portées sont respectées (`get_reader_roster` garde sur la même bibliothèque que son paramètre ; les `fn_my_*` sur `auth.uid()`). Le trouvé est ailleurs : six fonctions — cinq rapports de qualité du catalogue et `fn_authority_list` — refusaient par un `RETURN;` nu, rendant « pas le droit » et « rien à voir » indiscernables. Sur un rapport de qualité, une liste vide signifie « le catalogue est sain » : un refus déguisé en bilan rassurant (`DOC-SILENCE-1`). L'incohérence interne le prouve — les quatre `conv_*` du même schéma lèvent en `42501`. Portée mesurée et inégale : `fn_authority_list` est **atteignable par l'interface** (`/atelier-autoridades` n'a pas de garde de rôle), les cinq autres seulement en appel direct. Corrigé par substitution **vérifiée** (`20260901082124`, essayée à blanc en prod avant écriture) sans toucher aux droits ; gardé par une suite qui vérifie la **forme** et attrapera la septième. **60 des 138 lues.** **01/09, paquet 4 — 45 écritures sur objet : aucune faille, et c'est un résultat.** Trois paquets d'affilée avaient produit une prise ; celui-ci n'en produit aucune, sur la forme pourtant la plus dangereuse (agir sur la chose d'autrui). Les 45 suivent une doctrine implicite cohérente, constatée et désormais écrite : **la garde suit la propriété de l'objet** — propriété vérifiée pour ce qui est d'une personne (`v_owner <> v_uid`), garde par bibliothèque pour ce qui est d'une bibliothèque, rôle de catalogage sans bibliothèque pour les communs du réseau (notice, sujet, revue), admin réseau pour le fédéral. Trois formes citées à imiter, dont `attach_exemplar` qui **ne prend pas la bibliothèque en paramètre** (elle la déduit de l'appelant — héritage de l'incident de juillet) : la garde la plus sûre est celle qu'on ne peut pas contourner parce que le paramètre n'existe pas. **105 des 138 lues, 33 restantes.** **01/09 — le lot `api` est CLOS : 138 sur 138 ont un verdict écrit**, bouclage vérifié par le second chemin (`DOC-RECENS-1`) — zéro non-lue, zéro nom fantôme. Le paquet 5 (les 32 restantes : diffusions, réglages, gestes sur soi) n'a rien trouvé, et les deux actes les plus lourds du réseau sont les mieux gardés (`fn_gazette_broadcast` et `fn_lettre_issue_send` exigent `network_staff`). **Bilan du lot : deux fuites réelles, six silences, zéro défaut sur les 77 écritures et réglages** — et les deux fuites étaient dormantes, l'une attendant l'activation des cotisations, l'autre le premier gel réseau. La leçon retenue : *la question utile n'est pas « est-ce exploitable aujourd'hui ? » mais « qu'est-ce qui l'armerait ? »*. Le motif commun aux trois prises : une garde qui vérifie une relation que la requête suivante n'utilise pas. **01/09 — le schéma `public` est entamé (69 des 326 lues), et la première prise corrige mon propre correctif du matin.** `api.get_due_date_for_loan` avait été bornée ; **le helper qu'elle appelle, `fn_is_loan_blocked_by_dues`, restait exposé** — on posait la même question directement. *Corriger un chemin ne corrige pas ce qu'il traversait* : `DOC-RECENS-1` appliqué aux correctifs. Garde posée dans le corps, vérifiée en prod sur les trois chemins (concernée : répond ; staff : répond ; tiers : `42501`). **Et un oracle non dormant, exploitable avec un simple compte** : `fn_painel_find_profile_by_lookup` distinguait « compte trouvé mais pas chez vous » de « rien trouvé » — un test d'existence d'adresse e-mail dans un réseau militant ; message unifié, doctrine de `resolve_reader_card`. Quatre helpers internes fermés au passage (dont `get_library_contact_for_cooperation`, qui rendait courriel, téléphone et adresse postale de n'importe quelle bibliothèque **sans aucun appelant**) : 326 → 322 exposées. Migration `20260901085639`, CI verte. **01/09, paquet 2 de `public` — 19 nominatives lues, et quatorze sœurs d'un même oracle.** `fn_painel_get_profile_by_id` était la jumelle exacte de la fonction corrigée le matin : j'avais corrigé sans chercher les sœurs. Le motif en a révélé une deuxième (`fn_attach_received_asset_record`, identifiant **séquentiel** — on compte les fonds reçus en incrémentant), puis **douze de plus** quand la suite de test a cherché le message *sans ses accents* (« nao pertence ») : toute la famille `fn_import_*`. Corrigées par substitution vérifiée (`20260901091431`), aucun corps recopié. **Cette migration a coûté trois rouges sur `main`** : un `DEFAULT` omis en recopiant un corps, un correctif poussé sans être éprouvé, puis une suite d'août qui assertait le libellé du message changé. Trois règles au REGISTRE (`DOC-MSG-1`) — un message d'erreur est un contrat ; ne pas recopier un corps ; une cause certaine se vérifie quand même. **88 des 326 lues.** **01/09, paquet 3 de `public` — 57 listes à paramètre de portée : aucune fuite.** Les portées sont respectées, y compris sur la forme à risque (les dépôts de garantie, appelés par un identifiant séquentiel, filtrent bien sur soi-même ou staff). Cinq faux positifs de mon détecteur de refus muet, et la distinction est écrite : *un `RETURN;` après un paramètre vide est légitime, après un refus non*. **Une question de doctrine posée plutôt que tranchée** : quatre fonctions réseau mettent leur garde dans le `WHERE` — un appel non autorisé rend zéro ligne au lieu d'une erreur, la forme corrigée au paquet 3 d'`api`. Mais ici c'est **délibéré et commenté**, donc laissé au collectif : soit elles rejoignent la règle, soit `DOC-SILENCE-1` gagne une exception écrite pour les gardes en `WHERE`. **138 des 326 lues, restent 188.** **01/09, correction de mon propre compte** : j'avais écrit « 145 des 326 » en additionnant les paquets comme si leurs critères s'excluaient — sept fonctions relevaient de deux critères et étaient comptées deux fois. Le vrai chiffre était **138**. *Un reste mesuré est une mesure ; une somme de paquets est une estimation* — on déduit les lues du reste, jamais l'inverse. **01/09, paquet 4 de `public` — deux fuites, et le piège du remède uniforme.** Les 188 restantes retriées avec le **vocabulaire réel des gardes** (21 prédicats extraits par introspection des `IF NOT <appel>`, dont six que mes listes à la main avaient manqués) : 39 sans garde connue. `fn_next_tombo` n'en avait aucune et rend le prochain numéro d'inventaire d'une bibliothèque — donc **le nombre d'exemplaires qu'elle a déjà catalogués** ; en boucle sur le réseau, c'est la volumétrie comparée des fonds. `link_book_contributors_to_authors` n'en avait aucune non plus **et elle écrit**. Mais le contrôle des appelants, fait avant d'écrire, montre que **les deux sont appelées par les écrans de catalogage** : un `REVOKE` y aurait remplacé un refus par un écran mort, le piège de `confirm_pickup_v1`. Garde dans le corps pour elles (`DOC-RPC-3`), révocation pour les deux qui n'ont aucun appelant en session. *Le choix entre garder et révoquer se lit chez les appelants, jamais dans la fonction seule.* Éprouvé en production en transaction annulée dans les **deux** sens — lecteur refusé, staff passant — puis le fichier entier plus sa suite, `B14_GARDES_ECRITURE OK : 4/4`, production vérifiée intacte après coup. **177 des 322 exposées lues, restent 145.** **01/09, paquet 5 de `public` — le décalage n'était pas dans les fonctions.** Ce paquet cherchait, parmi les écritures gardées, l'écart entre l'objet gardé et l'objet écrit (la faille exemplaires/holdings de juillet). Les fonctions sont saines, et toutes de la même forme : *la garde se calcule à partir de l'objet lu, jamais d'un paramètre* — `fn_record_deposit` et `fn_record_membership_payment` n'acceptent même aucune bibliothèque en paramètre. Le décalage était dans la vue qu'elles interrogent toutes. **`api.my_access` répondait à deux questions comme si c'en était une** : `can_access_painel` disait « as-tu un rôle staff QUELQUE PART ? » et `library_id` disait « quelle est ta bibliothèque PRINCIPALE ? ». **37 fonctions lisent les deux ensemble, dont 24 qui écrivent** (circulation, cotisations, dépôts, import) : une personne bibliothécaire à A et simple lectrice à B, avec B pour principale, obtenait le panneau de B. **Démontré en transaction annulée** en armant le cas : la vue d'alors répondait `btl painel=true`, la corrigée répond `blmf painel=true role=librarian`. Une personne est concernée aujourd'hui, sauvée par le tri ; ce qui l'armerait est *un geste ordinaire de l'interface* (désigner l'autre bibliothèque comme principale). Correctif à la source, sans toucher aux 37 : l'adhésion effective préfère un rôle staff, et `can_access_painel` se calcule sur celle-là — **équivalent à l'ancien calcul, donc personne ne perd un accès, par construction et pas par chance**. La forme à imiter était dans le même paquet : `resolve_managed_library_id` **revérifie** le droit sur la bibliothèque venue de `my_access` au lieu de lui faire confiance — *une valeur qui vient d'une vue de session est une candidature, pas une autorisation*. **Et mon propre tri était trop étroit** : 53 écritures listées à partir de douze prédicats devinés, 67 selon le vocabulaire introspecté — le défaut que le paquet 4 venait de corriger, refait un paquet plus tard ; le second chemin n'est pas acquis une fois pour toutes. **Un écart de doctrine mesuré et laissé à décider** : `merge_book`/`merge_author` exigent l'arbitre (admin réseau ou coordenador), `merge_serial` et ses deux sœurs acceptent tout `librarian` — les périodiques du 27/08 n'ont pas repris la décision du chantier DOUBLONS P4. **4 personnes** sont `librarian` sans être `coordenador` : elles peuvent détruire une autorité de revue, pas une autorité de livre. Non corrigé — retirer un pouvoir à quatre personnes sans les prévenir n'a pas sa place dans un déploiement automatique. **01/09, les deux questions ouvertes ont ete tranchees le jour meme.** **(1) Les gardes en `WHERE` rejoignent `DOC-SILENCE-1`** : les cinq fonctions reseau levent desormais 42501 au lieu de rendre une liste vide. Forme inattendue mais imposee par le code — elles sont en `LANGUAGE sql`, donc pas de bloc `IF` ; les convertir aurait demande de reecrire cinq corps pour n’y changer qu’une garde, ce que `DOC-MSG-1` interdit. On garde le `WHERE` avec un **predicat levant**, apres avoir verifie le seul point qui compte : *une garde levante dans un `WHERE` se declenche-t-elle sur relation VIDE ?* Oui (mesure). La limite est ecrite — cela depend du plan — d’ou une suite qui **appelle** les fonctions au lieu de relire leur corps. **Piege symetrique evite** : dans `list_catalog_libraries` et `fn_team_list_invitations` le meme predicat ELARGIT un acces, et une substitution par motif y aurait transforme la navigation en erreur — correction nominative, gardee dans les deux sens. **Cout : un quatrieme rouge, et une regle elargie.** Deux suites attestaient le comportement retire (`mentions_orphelines` T10, `invitation_claims_lot2` T18). J’avais applique `DOC-MSG-1` aux **messages** et pas au **comportement** : *passer de « rend du vide » a « leve » casse tout autant ce qui l’atteste.* Corrige apres verification que tous les autres appels tournent sous JWT admin — pour ne pas enchainer un second rouge sur une seconde supposition ; le log de CI l’a confirme sans ecart. **(2) L’arbitrage des periodiques rejoint celui des livres, APRES preavis** : les trois fonctions passent a `fn_is_dedup_arbiter()`. Cataloguer et **signaler** restent ouverts au catalogage (DOUBLONS P8) — reperer un doublon et trancher un doublon sont deux actes differents. **La migration est ecrite, eprouvee et volontairement NON poussee** : elle retire un pouvoir a quatre personnes nommees, et le depot a deja pose la regle — changer ce qui appartient a quelqu’un sans le lui annoncer n’a pas sa place dans un deploiement automatique. Preavis redige en pt-BR, la langue des quatre. **01/09, paquet 6 de `public` — ce qui trahit n’est pas le mot, c’est l’ordre.** Le paquet 2 avait ferme quatorze oracles d’existence en cherchant le MOTIF du message (« nao pertence », puis la variante sans accents). Ce paquet en trouve **neuf de plus, et aucune ne dit « pertence »** : elles disent « Acesso restrito », « Voce nao tem permissao », « Este emprestimo pertence a outra pessoa ». *Chercher un vocabulaire ne trouve que ce qui parle la meme langue.* Le critere qui les atteint est **structurel et mesurable** : un test d’existence place AVANT le controle de droit, sur un identifiant devinable — releve en comparant la position des deux refus dans chaque corps, sur toute la surface exposee. Seize fonctions dans ce cas, **neuf sur identifiant sequentiel** (assets numeriques, runs d’import, jeux de regles de circulation, exemplaires, emprunts). **On n’inverse pas l’ordre** : la garde porte sur la bibliotheque DE L’OBJET, qu’il faut avoir lu pour la connaitre — c’est meme la bonne forme, celle que le paquet 5 a designee comme modele. On unifie donc ce que les deux refus DISENT, et l’identifiant sort du message. **Une correction du paquet 2 etait a moitie faite** : `fn_attach_received_asset_record`, corrigee le matin meme, avait garde un troisieme message de refus — et un seul suffit a rouvrir l’oracle. La suite du matin passait au vert parce qu’elle demandait « le motif a-t-il disparu ? » ; la bonne question est « reste-t-il DEUX refus distinguables ? ». *Une fonction corrigee n’est pas une fonction close.* **Un defaut de ma substitution attrape a l’essai a blanc** : deux fonctions voisines partagent un refus sans partager leur objet (l’une parle d’un « Asset », l’autre d’un « Recurso digital »), et un remplacement global faisait dire a l’une le message de l’autre. Substitution ciblee par fonction. **Hors perimetre et dit** : les identifiants `uuid` (un uuid ne se devine pas ; corriger serait du bruit, et une suite qui rougit sur des cas sains cesse d’etre lue) et les refus de ROLE qui tombent avant toute lecture. `DOC-MSG-1` applique dans sa forme elargie du matin — aux messages ET au comportement : aucune assertion sur les huit libelles, ni en tests ni au front. **01/09, paquet 7 de `public` — fermer ce que personne n’appelle.** Derniere classe, la plus sensible dans un reseau militant : les lectures qui rendent des donnees NOMINATIVES. Sur 211 lectures exposees, 17 touchent `profiles`, 14 rendent une identite — treize sont gardees (dont `fn_list_membership_payments_for_user`, exemplaire : panneau exige ET filtre sur la bibliotheque active). **La quatorzieme n’a aucune garde** : `fn_assembleia_facilitator_name` rend prenom et nom pour un identifiant donne, et ne repond que si la personne est facilitatrice — elle joint une identite a un ROLE MILITANT. `CLAUDE.md` la cite parmi les « fuites reelles fermees » du 17/08 : elle l’avait ete pour `anon`, et personne n’avait regarde `authenticated`. *Une fuite fermee d’un cote reste ouverte de l’autre tant qu’on n’a pas nomme le cote* — c’est l’objet meme de B14, et son meilleur exemple. **Second critere : ce que personne n’appelle.** Une RPC est appelee par le FRONT, donc « sans appelant en base » ne dit rien seul ; croise avec le depot, il reste **53 fonctions exposees que rien n’appelle nulle part**. *Ce critere aurait pu etre faux et a ete eprouve avant d’etre cru* : si un nom de RPC etait construit par concatenation, aucune recherche textuelle ne le verrait — verifie, aucun ne l’est. **Les 53 ne sont PAS revoquees** : beaucoup attendent leur ecran (tout le pret entre bibliotheques, le parcours de candidature), les fermer casserait un chantier au lieu de proteger. Liste portee a l’audit. **Cinq sont fermees**, celles que le depot lui-meme dit depreciees ou remplacees et qui touchent a l’identite. **Une ironie qui vaut lecon** : les deux fonctions du panneau fermees aujourd’hui avaient recu le matin meme le correctif d’oracle du paquet 2 — *le correctif etait juste, la cible ne l’etait pas*. Demander qui appelle vaut mieux AVANT d’ecrire la migration que trois paquets plus tard. **Une garde en WHERE reste, et c’est motive** : `fn_network_resolve_public_id` rend NULL plutot qu’une erreur, et son commentaire l’argumente — *une liste vide affirme « il n’y a rien », une valeur nulle dit « pas de resultat »* ; la premiere est fausse pour qui n’a pas le droit de voir, la seconde est vraie dans les deux cas. La decision du matin visait les listes qui mentent ; l’appliquer ici en ferait un reflexe. **01/09, paquet 8 de `public` — agir ou lire pour autrui : rien a corriger.** Critere : les fonctions qui acceptent un IDENTIFIANT DE PERSONNE en parametre, la classe qui avait donne la prise du lot `api`. **19 fonctions**, dont 11 qui ecrivent ; cinq ne citent meme pas `auth.uid()`. **Deux sont des pierres tombales** — `fn_team_promote_to_coordenador` leve `collegiality_required: direct promotion to coordenador is disabled` avec un HINT qui indique le parcours collegial en trois temps, et `fn_network_admin_request_removal` renvoie au vote a l’unanimite. Ce ne sont pas des fonctions abandonnees mais des **decisions de gouvernance materialisees dans le code**. *Une fonction dont le corps ne fait que lever est un faux positif structurel de TOUT critere « sans garde » : elle n’a rien a garder.* Le detecteur doit les exclure, sinon il crie a chaque decision bien prise. **Le modele du schema est dans ce paquet** : `fn_painel_reader_other_memberships` rend les autres rattachements d’une lectrice — l’information la plus sensible qu’un reseau militant puisse detenir — et ne les nomme que sous **trois conditions cumulatives** : partenariat actif, droit `transparence` explicitement accorde, et **consentement valide de la personne**. Sinon la ligne revient a NULL : l’ecran dit « il y a autre chose et tu n’as pas le droit de le voir », ce qui est vrai, et laisse a la personne la decision d’en dire plus. *Le consentement n’est pas une case en plus de la garde, c’est un terme de la garde.* **Aucune faille sur les 19** — un paquet sans correctif n’est pas un paquet sans resultat, c’est ce qui permet de dire que la classe est passee. Le lot `api` avait connu la meme chose a son paquet 5 : le signe que la surface converge. **01/09, paquet 9 de `public` — la salle des machines.** Cartographie EN TRANSITIF de ce qu’un compte `authenticated` peut atteindre qui touche a l’exploitation : HTTP sortant (`pg_net`), secrets (`vault`), mecanique cron, files de notification — la classe de la toute premiere fuite du projet (`fn_gazette_translate_call`). Le scan direct ne suffisait pas : `fn_send_weekly_report_now` lit ses secrets via un wrapper que le motif `vault\.` ne voit pas dans l’appelante — meme lecon que le paquet 6, le critere etait le GRAPHE d’appels, pas le corps. **Verdict : aucune faille, et c’est le meilleur resultat possible.** L’architecture est en couches et les couches sont etanches : les ~30 fonctions moteur (`fn_dispatch_*`, `fn_enqueue_*`, `fn_cron_*`, `fn_internal_get_vault_secret`, les appels LLM) sont TOUTES fermees a `authenticated` ; les 44 fonctions metier exposees ne les atteignent qu’en tant que DEFINER — le lectorat declenche des consequences, jamais la mecanique ; deux exceptions font de l’HTTP en direct, chacune gardee sur sa bibliotheque, URL et secret venus du vault et jamais de l’appelant. `fn_gazette_translate_call` verifiee fermee : la boucle du 17/08 est bouclee des deux cotes. **Mais rien ne gardait cette etancheite** — un GRANT distrait sur `fn_internal_get_vault_secret` aurait donne TOUS les secrets du vault a tout compte du reseau, sans voyant. D’ou `salle_des_machines_tests.sql`, structurelle et non nominative : le critere se remesure a chaque passage et attrape les fonctions qui n’existent pas encore ; seules les deux exceptions HTTP sont nommees, et le T3 verifie qu’elles gardent leur garde — *une exception sans garde n’est plus une exception, c’est un trou*. SALLE_DES_MACHINES OK 4/4. **Et l’arbitrage des periodiques est deploye** : preavis envoye par la coordination le 01/09 aux quatre personnes, migration retenue en local jusqu’a cet envoi puis poussee (rehorodatee `20260901145725`), suite `arbitrage_periodiques_tests` en CI. La regle — retirer un pouvoir ne part jamais sans que les personnes aient ete prevenues — a ete tenue dans l’ordre qu’elle exigeait. **01/09, paquet 10 de `public` — les documents numeriques : la chaine est saine, son talon est le rangement.** Audit de tout ce qui donne acces aux fichiers, policies `storage.objects` comprises (16 buckets). **Une fausse piste instructive** : la policy de `pdf-restrito` n’admet que le staff alors que la RPC promet l’acces aux lectrices `conta_ativa` — j’ai cru la promesse cassee. C’est l’EF `read-digital-asset` qui la tient, et l’architecture vaut d’etre ecrite : la RPC decide AVEC LE JWT DE L’APPELANTE, puis l’EF signe en `service_role` (TTL court) — *la policy storage n’est pas le portier du lectorat, la RPC l’est*. L’EF masque meme `source_url` d’un document restreint : la provenance est elle-meme une information. **Le PEB numerique est un modele** : `fn_ill_signed_url` revalide le droit de partenariat AU MOMENT de l’acces — rompre le partenariat coupe les recus deja transmis (PARTNER-D5) ; *un droit qui ne se revalide pas a l’acces n’est revocable que de nom*. Et `digital_assets` ne recoit que des ressources publiques : le dispositif garde l’acces au service, pas des octets secrets. **Le talon, que rien ne gardait : le rangement.** Un asset `conta_ativa` catalogue par erreur dans un bucket public serait servi par l’URL publique — sans RPC ni EF, mondialement — et RIEN ne casserait : le parcours normal continuerait de fonctionner. *Ce qui rend l’exposition silencieuse est precisement que rien ne casse quand elle se produit.* Suite `documents_numeriques_tests` (4/4 en prod) : aucun asset non-publico en bucket public, les trois buckets restreints gardent `public=false` (une bascule = un clic de dashboard = tout expose), la RPC garde ses deux conditions — l’EF signe tout ce qu’elle rend — et la revalidation PEB reste en place. **Deux observations sans correctif** : 4 objets de `anarbib-media-restricted` inatteignables par quiconque (reliquat probable), et la lecture directe de `pdf-restrito` ouverte au staff de n’importe quelle bibliotheque — coherent avec la confiance reseau du catalogage, mais c’est un choix, et il est maintenant ecrit.*
...
- ~~Les 138 fonctions de `api` ont un verdict écrit : légitime, à restreindre, ou à supprimer~~ — 138/138 au 01/09, bouclage vérifié dans les deux sens (`AUDIT_execute_authenticated_2026-09-01.md`).
- ~~Les fonctions à restreindre ont perdu leur `EXECUTE` par migration, après vérification qu'aucune policy ne les appelle~~ — traitées autrement et mieux : aucune n'a perdu son `EXECUTE` (le refus vit dans le corps, doctrine `DOC-RPC-3`), trois migrations ont borné les gardes fautives (`20260831201011`, `20260901075511`, `20260901082124`).
- ~~Le compte rendu vit dans `docs/journal/audits/`, au format de celui du 18/05~~ — `AUDIT_execute_authenticated_2026-09-01.md`, cinq paquets datés.
- Le reste de `public` suit, par paquets — **145 des 326 lues au 01/09**, restent 181. Un arbitrage en attente : les quatre gardes en `WHERE` (refus muet délibéré) rejoignent-elles `DOC-SILENCE-1` ou gagnent-elles une exception écrite ?
...
*Vérifié : 31/08 — 89 sujets et 98 alignements en base ; `docs/drafts/20260828_sujets_solidaires_ficedl.sql` est toujours hors de `supabase/migrations/`. Le constat tient tel quel.*
...
*Vérifié : 31/08 — le cron `anarbib-oai-harvest-weekly` (`20 4 * * 2`, actif) n'a aucune exécution à son journal. Première occurrence attendue le mardi 01/09 à 4 h 20.*
