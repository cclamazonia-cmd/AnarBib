...
| Tests | **349 + 62** | **349 cas JavaScript** dans 25 fichiers — chiffre donné par l'exécution, non par un grep des déclarations. **62 suites SQL, toutes en CI** — neuf sont nées du seul audit B14, clos le 01/09 |
...
- Le trafic navigateur à préfixe `apikey` vide est resté proche de zéro sur plusieurs jours consécutifs de mesure.
...
*Vérifié : 31/08 — trois `Resolve-DnsName` (TXT du sous-domaine, `resend._domainkey`, `_dmarc`), plus les emplacements `send.` et le parent : SPF porté par `send.` (SES `eu-west-1`), DKIM publié, DMARC `p=none` avec `rua` Brevo aux deux niveaux, `brevo-code` résiduel. **Relevé fait le 01/09/2026** (Resolve-DnsName, lecture seule). **En règle pour envoyer** : SPF porté par le sous-domaine Resend (`send.notifications` : `v=spf1 include:amazonses.com ~all` + MX feedback SES) et DKIM présent (sélecteur `resend`). **Les deux défauts annoncés sont confirmés** : (1) DMARC en `p=none` avec `rua=mailto:rua@dmarc.brevo.com` — sur le sous-domaine ET sur `anarbib.org` : les rapports d'authentification partent chez le prestataire quitté, qui les lit à notre place ; (2) résidus Brevo (`brevo-code:…`) sur les deux zones, plus un `include:mx.ovh.com` sur la racine. Reste à faire, deux gestes DNS chez OVH : pointer le `rua` vers une boîte du réseau (`admins@anarbib.org`, déjà destinataire des alertes santé) en gardant `p=none` le temps de lire quelques rapports, et purger les deux TXT `brevo-code`.*
