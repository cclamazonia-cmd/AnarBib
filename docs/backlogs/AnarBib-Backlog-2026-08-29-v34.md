...
| Migrations appliquées | **311** | 311 migrations numérotées au dépôt = **311 appliquées, alignement exact** (vérifié le 07/09 au soir). **+3 le 07/09** : `20260907120000` (H8 — GetRecord sert la notice demandée), `20260907123000` (I20 — `anarbib.functions_base_url`, l'adresse des fonctions n'est plus codée en dur ; posée par l'étape « 5 bis » de `bootstrap.sh` sur une instance auto-hébergée), `20260907172508` (l'alignement FICEDL dit vers quelle liste il pointe — H10/H12). |
...
| | | |
|---|---:|---|
| Commits | **2 621** | sur `main`, au 07/09 au soir — 27 commits dans la journée, de deux sessions : le code de H8 et I20, la relecture de la FICEDL (H9–H13), le cadrage du rejeu fidèle (I17, I21, I22), dix-huit items et trois clôtures au backlog. |
| Fichiers `src/` | **311** | 81 pages, 93 composants ; +1 fichier le 07/09 : le test `migrations-sans-url-cloud.test.js` (I20 — aucune migration ne doit porter l'adresse du projet cloud). |
...
| Tests | **411 + 97** | 411 tests JS (vitest, gate bloquant) + **97 suites SQL** dans `ci-suites.txt` (+2 le 07/09 avec H8 et I20, après `hygiene_search_path_et_initplan` le 06/09). Rejeu local des migrations le 06/09 ; CI verte sur les trois pushes du jour. |
