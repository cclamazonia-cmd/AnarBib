...
- **COTIS-9 — paiement self-service : par bibliothèque, pas de plateforme unique.** Le « prestataire éthique unique » n'a pas à exister — la cotisation se règle sur le **rail local** de chaque biblio (le paysage est fragmenté par pays). Deux étages :
  - **(a) Coordonnées de paiement par biblio + confirmation manuelle** — *cadré, fidèle à COTIS-2/3/7, activable à tout moment, sans intermédiaire/frais/périmètre PCI.* Un champ de config par biblio (texte libre + structuré optionnel) affiché sur `/conta` : **IBAN** (→ AnarBib génère le **QR code EPC** via la dépendance `qrcode` déjà présente ; IBAN copiable en repli), **handle Wero** (tel/e-mail, sans IBAN), **clé/QR PIX** (Brésil), ou **URL HelloAsso / Open Collective / Liberapay**. La lectrice paie par le canal **choisi par sa biblio** — au Brésil **PIX** (rail public gratuit instantané) ; en France/zone € **virement instantané SEPA** (gratuit et obligatoire depuis 2025 ; +QR EPC quand l'appli bancaire le gère ; Wero pour le tel/e-mail) — puis le **staff confirme** (flux d'enregistrement actuel, COTIS-3).
  - **(b) Encaissement automatique (« clic → enregistré »)** — *ouvert.* Webhook d'un prestataire, **par biblio, opt-in, jamais imposé**, à n'ouvrir qu'avec un acteur **éthique/militant** (HelloAsso = France seule ; Open Collective Europe = transparent mais ~10 % + fiscal host ; Liberapay = 0 % de commission mais orienté don récurrent) et, si la portée est réseau, une décision d'AG. Wero (récurrent prévu 2026-27) à surveiller comme rail A2A européen.
