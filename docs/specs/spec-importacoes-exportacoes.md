...
**IMP-11 — Adaptateurs (hybride).** Registre **déclaratif** **`ingest.import_adapters`** (combinaisons structure × vocabulaire connues → profil par défaut + statut) ; la **logique de décodage vit en code** — déjà : l'EF parser (CSV/TSV/RIS) + `detected_format` + `parser_version`. **Détection auto = la structure** (sniff CSV, signature RIS, magie ISO 2709, racine XML, forme JSON) ; le **vocabulaire** est déclaré/choisi. Combinaison inconnue → repli **mapping manuel** (`IMP-4`). **Priorité d'implémentation** : (a) **CSV/TSV · RIS** (déjà parsés) + **Zotero (CSL-JSON)** + **UNIMARC ISO 2709** (migration BLMF réelle, parser à ajouter) ; (b) Dublin Core · MARCXML · OAI-PMH ; (c) MARC21 · BIBFRAME.
...
**IMP-14 — Rôles.** **`coordenador`** : lance le wizard, promeut, exporte, gère les relations partenaires + `digital_share` (`PARTNER-D7`). **`librarian`** : consulte + imports fichier simples (espace staff, pas publication). Ratification au foyer `spec-gouvernance-roles`. ⚠️ **À durcir (cf. cadrage 08/06)** : les RPC `ingest.*` sont aujourd'hui granted à **`service_role` seulement**, le garde-fou est **purement frontend** (`canImport`) ; ajouter `GRANT … TO authenticated` + **validation de rôle dans la RPC** (`user_can_act_as_staff_on_library`), conformément à `DOC-RPC-3`.
