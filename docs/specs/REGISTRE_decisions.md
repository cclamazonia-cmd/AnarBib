...
|**DOC-EF-BUNDLE-1**|Edge Functions déployées par **bundle autonome par fonction** : `supabase/functions/<fn>/_shared/…` est un **sous-ensemble maintenu à la main** des modules que la fonction importe (`./_shared/...`). Le `supabase/functions/_shared/` de **premier niveau** = **bibliothèque de référence NON déployée** (exclue par la boucle `deploy-edge-functions`). **Corollaire** : ajouter un import à un handler exige de **copier physiquement le module dans le bundle de SA fonction**, sinon l'import casse au déploiement.|✅ acté 04/06|constat session i18n mails de tâches (`task-mail-strings.ts` posé dans `notify-internal-task/_shared/i18n/`) ; cf. DOC-DEPLOY-1|
|**DOC-OUTBOX-1**|**`dispatch_status` n'est pas une preuve de livraison.** Les dispatchers posent `'sent'` juste après `net.http_post` (asynchrone) = « **émis** », pas « livré » ; l'issue HTTP réelle (401/500/timeout) n'arrive qu'ensuite dans `net._http_response` (`id = request_id`). Vérité = **réconcilier** : cron `reconcile-task-dispatch` (`*/5`) rabat les faux `'sent'` (non-2xx / null / timeout) → `'failed'` + `last_error`, et estampille `reconciled_at` sur les `2xx`. **Lecture** : `'sent'` + `reconciled_at` rempli = confirmé ; `'sent'` nu = émis, non confirmé.|✅ acté 04/06 (migration `reconcile_task_dispatch_status` ; `fn_cron_reconcile_task_dispatch`)|incident d'origine : `401` d'auth masqué par un `'sent'` menteur (env `WEBHOOK_SECRET_NOTIFY_INTERNAL_TASK` ≠ vault `ANARBIB_TASK_INVITE_SECRET`) ; cf. DOC-NOTIF-1|
|**DOC-MAIL-LOCALE-1**|**Langue d'un mail = résolue du contexte du·de la destinataire**, jamais codée en dur. **Avis internes biblio** (tâches, etc.) = `libraries.default_locale` (repli pt-BR) — langue d'équipe, vaut aussi pour l'invité·e non inscrit·e. **Mails lecteur·rice** = `preferred_language` (cf. MYLIB-5). Précise DOC-I18N-1 (« 9 locales ») en disant **laquelle** selon l'audience. Strings via `tMail` / tables `_shared/i18n/*-mail-strings.ts`.|✅ acté 04/06|i18n mails de tâches (`task-mail-strings.ts`) ; cf. DOC-I18N-1, MYLIB-5|
...
|**FED-O4**|**Tranché** : terme objet = **`círculo`** (seul à couvrir les 4 natures — affinitaire / géographique / linguistique / fédération ; *coletivo* ambigu, *afinidade* trop étroit) ; label du bloc = **Ferramentas federalistas**.|✅ tranché 04/06|cadrage §6 (FED-2) ; spec-outils-federalistes|
|**FED-O5**|**Tranché** : adhésion à un cercle = **accueil par défaut (opt-out)**, *pas* cooptation (le cercle n'ouvre aucun droit — FED-3 ; le silence vaut consentement). Coordenador candidat → adhésion `pendente` notifiée aux coordenadores des biblios membres ; **objection motivée** (motivation longue obligatoire, visible, tracée) pendant un **délai (~14 j, à caler)**. Effet = **anti-blackball (B)** : une objection isolée **suspend + ouvre la discussion**, ne vaut refus que si **une 2ᵉ biblio membre la rejoint** dans le délai ; cercle minuscule (1-2 membres) → l'objection d'un membre bloque. Réemploi de la grammaire cooptation, en régime opt-out.|✅ tranché 04/06|spec-outils-federalistes §5.5|
|**FED-O6**|**Tranché (orientation)** : la mutualisation de catalogue est un **axe distinct** de la visibilité de fiche — elle ne touche **pas** `visibility_level` ni `fn_library_visible_to_caller`. **Opt-in au niveau biblio, multi-cercles** (future `library_catalog_sharing(library_id, circle_id)`) ; `books.mutualization_status` = **exception fine par notice** (`local_only` exclut ; `NULL` suit le réglage biblio) ; **réciprocité non forcée** (autonomie locale) ; **granularité (i)** = tout le fonds moins `local_only`. Filtrage effectif = **fonction dédiée** (*pendant catalogue* de `fn_library_visible_to_caller`), à écrire avec **catalogação**. Hors cœur cercle v0.1 ; la future table pointera vers `circles(id)` (FK ajoutée le jour venu). Mécanique fine renvoyée à catalogação.|✅ tranché 04/06|spec-outils-federalistes §5 ; cf. catalogação|

> *Les `FED-O*` sont les questions ouvertes du cadrage ; celles marquées ✅ sont tranchées (l'ID est conservé pour la traçabilité avec la spec). **Restent ouvertes** : FED-O1, FED-O2, FED-O3.*
...
*Fin du seed v0.1 (enrichi 02/06 : VALID-β1/γ1, sections MULTI, PARTNER, ILL, OPAC, ONBO ; doctrine DOC-COLLECTIVE-1 ; enrichi 03/06 : DOC-CIRC-1, USER-EMAIL-1, section CARD (carte-lecteur) ; enrichi 04/06 : section FED — réseau fédératif / outils fédéralistes ; FED-O4/O5/O6 tranchés le 04/06 ; section MYLIB (carte « ma bibliothèque » + messagerie réciproque) ; **section IMP (Importações — chantier-cadre cadré)** ; **carte γ complétée (UI staff livrée, gatée `reader_cards_enabled`)**). **Enrichi 04/06 (soir)** : DOC-EF-BUNDLE-1, DOC-OUTBOX-1, DOC-MAIL-LOCALE-1 (session mails de tâches : i18n sur la `default_locale` biblio ; réconciliation du `dispatch_status` menteur ; topologie des bundles EF par fonction). Décisions transverses recensées : 18. Drifts ouverts : voir le rapport d'audit joint.*
