...
| **FS-D3** | ⚠️ **Révisé 21/06** : la captation = **édition `books`** audio (pas de table `audio_recordings`) ; seules tables de granularité = **`audio_tracks`** + **`audio_track_contributors`** (FS-D5). | 🟡 forme DDL à confirmer en P1 |
...
| **FS-D5** | ⚠️ **Révisé 21/06** : crédits **grain édition** = `book_contributors` dérivés vers l'œuvre (patron `works_v3`) ; crédits **grain segment** = nouvelle **`audio_track_contributors`** (`authors` + rôles MB). | 🟡 à confirmer en P1 |
