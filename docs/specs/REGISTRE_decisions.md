...
| **PUBLIB-SCHED-1** | **Source canonique des horaires présentables = `library_opening_hours`** (multi-plages, libellée, publique via opt-in). `library_service_state.consultation_schedule_struct` reste **cantonné à la réservation de consultation sur place** ; `service_schedule_text` = notes internes. Résout la dette des **3 représentations** d'horaires (introduite le 17/06). Convergence consultation→`opening_hours` = **chantier ultérieur**, pas en urgence. | ✅ tranché (17/06) |
