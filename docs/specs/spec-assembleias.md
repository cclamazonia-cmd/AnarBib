...
| **Statut** | 🟢 **v0.1 + P2b + P2c livrées (16/06), EN PROD.** Backend : migrations `20260616191226` (P1 : dépôt ODJ) + `20260616214241` (P2b : rôle `assembleia_facilitators`, RPC d'animation re-gardées `network_admin` OU facilitateur·rice) + `20260616233952` (P2c : volontariat — `status` volunteer/designated, helper nom scopé, RPC volunteer/unvolunteer, désignation = promotion). Frontend **data-driven**, **10 locales** : **dépôt** d'un point (coordenador) ; **facilitation** (`isNetworkAdmin` OU facilitateur·rice désigné·e) — créer une assemblée, jalons J-30/J-15/~J-10/J-0, statut, différer, **ordonner l'ODJ** ; **volontariat** (se proposer) + **désignation parmi les volontaires** + retrait ; rotativité rappelée. **Chantier facilitation clos.** **P3 livrée (17/06), EN PROD** : migration `20260617004735` (émission best-effort `network.assembleia.{convocada,agenda_published,item_proposed}` via `fn_network_notify_event`) + handler EF `domain/assembleia.ts` (routé avant `network.*`) + `mail-strings` **10 locales** ; `convocada`/`agenda_published` → **chaque coordenador·e** des biblios fédérées (anti-rétention), `item_proposed` → **facilitation**. Vérifié MCP (migration appliquée, 3 RPC émettent, `notify-event` redéployée). **Chantier P3 clos.** **Reste** : choix de date, quorum 60/50, vote, ratification asynchrone : **v0.2** (§11). |
