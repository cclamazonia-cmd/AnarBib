...
| **Statut** | 🟢 **v0.1 + P2b livrées (16/06), EN PROD.** Backend : migrations `20260616191226` (P1 : 2 tables RLS, 2 vues, 7 RPC dépôt) + `20260616214241` (P2b : rôle `assembleia_facilitators`, helper, vue, RPC add/remove, **4 RPC d'animation re-gardées `network_admin` OU facilitateur·rice**). Frontend : onglet **data-driven** — **dépôt** d'un point (coordenador) + retrait ; **facilitation** (créer une assemblée [admin réseau], jalons J-30/J-15/~J-10/J-0, statut, différer) gardée `isNetworkAdmin OU facilitateur·rice` ; **10 locales**. **Reste** : UI de **désignation** des facilitateur·rices (+ ordonnancement ODJ) — P2b-suite ; **P3** (events notify-event). Choix de date, quorum 60/50, vote, ratification asynchrone : **v0.2** (§11). |
