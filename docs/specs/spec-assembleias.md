...
| **Statut** | 🟢 **v0.1 livrée (16/06)**. Backend **P1 EN PROD** (migration `20260616191226` : 2 tables RLS, 2 vues `api.*_v1`, 7 RPC `fn_assembleia_*`). Frontend **P2 EN PROD** : onglet **data-driven** (liste des assemblées + ODJ + **dépôt** d'un point gardé coordenador + retrait), 10 locales. **Reste P2b** (contrôle facilitation « créer une assemblée / poser les dates ») et **P3** (events notify-event). Choix de date, quorum 60/50, vote, ratification asynchrone : **v0.2** (§11) — dépendent de points ouverts (liste des zones notamment). |
