...
**Dernière mise à jour** : 1ᵉʳ septembre 2026 (nuit) — **v2 du saut collégial** (`GOUV-14`, spec **v1.7**, registre **v0.10**) : UI de réglage de gouvernance dans l'onglet Équipe (visible du staff, bascule coordenador+), mails du circuit qui **disent le saut** (intros `_direct` ×10 locales, détection côté EF, aucune migration), choix de l'atterrissage au self-demote (`reader` présélectionné pour les profils venus par saut) — *précédemment (soir) :* **le saut collégial est livré et T1 est tombée le même jour** : migration `20260901162610` en prod (opt-in `allow_direct_coordenador`, **BTL activée**), puis `20260901175233` — **l'accueil aussi est collégial** (`fn_team_promote_to_librarian` condamnée façon GOUV-4, le circuit d'invitation est le seul chemin ; rattrapage admin réseau d'une biblio sans staff = saut collégial, spec §6.4 réécrit). `spec-gouvernance-roles` passe en **v1.6**, registre **v0.9** (`GOUV-13`, `GOUV-11/12` livrées) ; les trois transitions montantes sont désormais collégiales — *précédemment (matin) :* **cadrage « saut collégial »** : `CADRAGE_promotion_directe_reader_coordenador_2026-09-01` (trace, `docs/journal/cadrages/`) évalue le passage direct `reader` → `coordenador` pour les biblios en collectif horizontal (cas BTL) — besoin, portée, conséquences sur la coordination, questions Q1–Q5. Arbitrages **`GOUV-11`/`GOUV-12`** actés au registre (v0.8) : opt-in **par biblio**, défaut désactivé, cible = reader `active` strict, circuit collégial intégral conservé ; chantier v1 ouvert → `spec-gouvernance-roles` passera en **v1.5**. L'incohérence T1 (promotion directe unilatérale `reader` → `librarian`) est posée au « reste ouvert » du §41, chantier séparé — *précédemment :* 29 août 2026 — passe d'hygiène du corpus (backlog v34, item J5) : **sept specs présentes au dépôt et absentes de cet INDEX** rendues navigables (nouvelle section « Specs présentes au dépôt… »), et la doctrine de déploiement corrigée (Woodpecker → **Forgejo Actions**, `DOC-DEPLOY-1`, migré le 11/06/2026). *Précédemment : **`spec-conventions-catalographiques` v0.1 créée** (🟡 cadrée) : le catalogue avait deux ans de saisie sans convention d'écriture — deux formes de `preferred_name`, des points d'accès pris sur une particule ou un suffixe de filiation (`FILHO, Fábio Luz`), 216 titres à mots-outils capitalisés, `idioma`/`country` non normalisés. Doctrine **`DOC-CONV-1`** (*une seule vérité en base, plusieurs rendus*) inscrite au **registre §0**, déclinée en **§37 `CONV`** (`CONV-1…CONV-7` + `CONV-O1…O4`) — registre passé en **v0.2**. *(Le cadrage visait §17 : déjà pris par `IMP`, `#HYG-REG-1` s'applique.)* Six migrations écrites, **aucune appliquée** ; trois tables de revue humaine créées vides de verdict (non automatisables). Constat : `journal/audits/AUDIT_conventions_catalographiques_2026-08-20.md` — précédemment : 20 août 2026 — **§ 36 `MOBILE` créé au registre** (`MOB-1..MOB-9` + `MOB-Q1..Q3`) : la doctrine responsive gradue enfin — garde `minmax(0, Nfr)` sur toute piste de grille, filet global `src/styles/mobile.css`, échelle de points de rupture 640/768/1100, tiroir mobile, champs à 16 px / cibles à 44 px, tables staff en cartes, et la règle de méthode « on mesure la vraie page, on ne lit pas le code ». Sort le responsive des « restes hors-specs » ; trace : `journal/cadrages/CADRAGE_mobile_responsive_2026-06-16.md` § 8 — précédemment : 21 juin 2026 — **`spec-fonds-sonores` v0.1 créée** (🟡 cadrée) : modèle d'import/catalogage des **œuvres audio** (fonds sonores) greffé sur le modèle d'entités **MusicBrainz** (Work/Recording/Track + MBID + empreinte Chromaprint), **sans importer ses données** ; foyer registre **§35 `AUDIO`** (`FS-D1…FS-D8` + `FS-Q1…Q5`) — précédemment : 5 juin 2026 (session soir) — **Track A catalogação complet** (Lots 1→6 + 3b) + **module capas P1/P2/P3** (P3 page-1-PDF **côté client**) ; **liaison autorités↔œuvres** (`fn_normalize_name`, trigger contributeurs→book_authors, RPC matching + outil « Rattacher aux œuvres ») ; **fusion de doublons** autorités **et** documents (`merge_log`, `merge_author`, `merge_book`) ; **flux contributeurs** brouillon→publié (triggers seed+sync, préservation `author_id`) ; fiche livre affiche tous les contributeurs. **3 specs nouvelles** : `spec-autorites-notes-bio-multilingues` v0.2, `spec-liaison-autorites-oeuvres` v0.2, `spec-doublons-detection-fusion` v0.1. Registre : `CAT-E7…E9, CAT-C5, CAT-G1/G2, CAT-H1, CAT-I1` — précédemment : 5 juin 2026 — chantier **Importações/Exportações** charpenté : `spec-importacoes-exportacoes` v0.1 créée (squelette, **§9 wizard cadré**) ; registre **§17 `IMP-1..8`** ; maquettes de référence (tableau de bord v7 + wizard v1) — précédemment : 4 juin 2026 (nuit) — carte-lecteur **phase γ complète** (UI staff `ResolveCardBox` livrée, **gatée `libraries.reader_cards_enabled`** : invisible là où la carte n'est pas activée — hotfix Woodpecker vert) ; **`CADRAGE_importacoes_module_2026-06-04` ouvert** (chantier-cadre Importações ; registre §`IMP` ; **défère** `ACQ-Q4` frontière Catalogação + `FED-5` rôle coordenador) ; `DOC-I18N-1` réaligné **9 locales** (`nl`) — précédemment : 4 juin 2026 (soir) — **`spec-outils-federalistes` v0.1 créée** (face fédération + primitive cercle `círculo` ; cœur socle + cercles) ; **arbitrages FED-O4/O5/O6 tranchés** au registre §`FED` (terme `círculo` ; adhésion opt-out + anti-blackball B ; mutualisation = axe distinct, opt-in biblio multi-cercles) — précédemment : 4 juin 2026 (**section `FED`** au registre : modèle d'accès concentrique + face fédération « Ferramentas federalistas », `círculos` relocalisé hors `rede` ; cadrage `CADRAGE_modele_acces_concentrique_2026-06-04.md`) — précédemment : 3 juin 2026 (soir) — **dette ponctuelle soldée** (verrou `get_accessible_digital_asset` selon DOC-OBJ-2 ; **USER-EMAIL-1** : `profiles.email` rendu miroir synchronisé de `auth.users.email`, registre §0) + **carte-lecteur v0.2** (résolution staff `api.resolve_reader_card` livrée backend, contrat des 3 RPC, section `CARD` au registre §23) — précédemment : 3 juin 2026 (`#MODEL-item-grain` **constaté livré en prod** via audit du dump schéma 03/06 → `spec-granularite-item` passe en 🔵 référence historique [cœur + suite `#ILL-availability`] ; ouverture Phase 1 exemplares, cf. `docs/journal/`) — précédemment : 2 juin 2026 (trilogie doctrinale : `spec-multi-appartenance-lecteur` v0.3 + `spec-partenariat-biblios` v0.3 figées, `spec-flux-partage-numerique` v0.2 charpentée ; registre enrichi MULTI/PARTNER/ILL/VALID-β1-γ1) — précédemment : 1er juin 2026 (réinjection du chantier catalogue : **6 specs** — 4 catalogage + 2 découverte lecteur — + ordre de mise en œuvre) — précédemment : statut #CL.8 actualisé ; 31 mai 2026 (audit du corpus, groupe A, refonte INDEX/INVENTAIRE)
...
- **Déploiement** = `git push` → **Forgejo Actions** (`DOC-DEPLOY-1/2/3`). *Migré de Woodpecker le 11/06/2026 ; toute mention de Woodpecker ailleurs dans le corpus est une trace périmée.*
...
| Gouvernance locale (rôles, status, transitions) | [`spec-gouvernance-roles.md`](./spec-gouvernance-roles.md) | v1.7 (01/09/2026) | 🟢 Référence — T2 collégiale (26/08) ; le 01/09 : **saut collégial livré en opt-in par biblio (v1.5, BTL activée), accueil T1 collégialisé (v1.6, promotion directe condamnée), v2 ergonomie (v1.7 : UI de réglage, mails du saut, atterrissage du self-demote)** — cadrage [`CADRAGE_promotion_directe_reader_coordenador_2026-09-01`](../journal/cadrages/CADRAGE_promotion_directe_reader_coordenador_2026-09-01.md), registre `GOUV-11`→`GOUV-14`. Les trois transitions montantes passent par le circuit d'invitation. |
...
| Modèle de catalogage des œuvres audio (fonds sonores) | [`spec-fonds-sonores.md`](./spec-fonds-sonores.md) | v0.1 (21/06/2026) | 🟡 Cadrée — greffe du modèle MusicBrainz (Work/Recording/Track + MBID + Chromaprint) sur `books`/`authors`/`book_digital_resources`, sous-couche **opt-in** ; registre §35 `AUDIO` |
...
| Conventions d'écriture (autorités & titres) | [`spec-conventions-catalographiques.md`](./spec-conventions-catalographiques.md) | v0.1 (20/08/2026) | 🟡 **Cadrée, non implémentée** — `DOC-CONV-1` (§0) + `CONV-1…7` (§37) inscrits au registre ; 6 migrations écrites **non appliquées** ; 3 tables de revue humaine |
...
### 🧩 Specs présentes au dépôt et absentes de la navigation ci-dessus

Sept specs vivaient dans `docs/specs/` sans être citées nulle part dans cet INDEX — donc introuvables autrement qu'en listant le dossier. Constaté et réparé le 29/08/2026 (backlog v34, item J5). Elles sont rangées ici plutôt que réparties dans les familles ci-dessus, parce que leur rattachement de domaine mérite d'être tranché plutôt que deviné.

| Domaine | Spec | Version | Statut |
|---|---|---|---|
| Acquisition & provenance des documents | [`spec-acquisition-provenance-v0_2.md`](./spec-acquisition-provenance-v0_2.md) | v0.2 (02/06/2026) | 🟢 Arbitrages Q1–Q6 + Q3-bis actés, prêt pour implémentation — supersède v0.1 |
| Assemblée du réseau (face fédération) | [`spec-assembleias.md`](./spec-assembleias.md) | v0.1 (16/06/2026) | 🟢 v0.1 + P2b + P2c **en prod** — foyer registre §32 `AG` |
| Dépôt de garantie (caution par livre / par emprunt) | [`spec-depot-garantie.md`](./spec-depot-garantie.md) | v0.1 | 🟡 Cadrage — rien d'implémenté, à trancher en collectif/AG avant tout code |
| Gazette (pull) & Lettre de la fédération (push) | [`spec-gazeta-lettre-federation.md`](./spec-gazeta-lettre-federation.md) | v0.1 (16/06/2026) | 🟢 Lots 2, 3 et 2b-bis **en prod** — foyer registre §29 `GAZ` |
| Fournisseur OAI-PMH & gouvernance de l'ouverture | [`spec-oai-provider-gouvernance.md`](./spec-oai-provider-gouvernance.md) | — (MAJ 16/06/2026) | 🟢 Implémentée & déployée (lots OAI-O1..O3) |
| Pseudonymisation des sauvegardes | [`spec-pseudonymisation-bg2-14.md`](./spec-pseudonymisation-bg2-14.md) | — (01/07/2026) | 🟡 Cadrée, arbitrages clos — incarne `BG2-14` (registre §BG2) |
| Ressources numériques | [`spec-ressources-numeriques.md`](./spec-ressources-numeriques.md) | — (19/06/2026) | 🟢 Implémenté — consolidation a posteriori de code en production |
...
- **`spec-conventions-catalographiques`** — **v0.1 🟡 cadrée le 20/08** (`docs/specs/`). Deux ans de catalogage sans convention d'écriture ont sédimenté des régimes contradictoires, mesurés sur staging (1 300 autorités / 2 674 notices) : deux formes concurrentes de `preferred_name`, points d'accès pris sur une particule (`DE BEAUVOIR, Simone`) ou un suffixe de filiation (`FILHO, Fábio Luz` — une autorité « Monsieur Fils »), doubles patronymes hispaniques scindés (`MAGÓN, Ricardo Flores`), 216 titres à mots-outils capitalisés, référentiels `idioma`/`country` non normalisés. Doctrine **`DOC-CONV-1`** — *une seule vérité en base, plusieurs rendus* (le point d'accès n'est ni la forme d'affichage ni la forme d'export ABNT) — déclinée en `CONV-1…CONV-7`, **inscrits au registre §37 `CONV`** (la doctrine elle-même en §0 ; registre passé en v0.2). Le §17 visé au cadrage était occupé par `IMP` — `#HYG-REG-1` a tranché le report. `CONV-6` (`authors.name_lang`) reste 🟡 à confirmer. **Six migrations écrites, aucune appliquée** (`00` snapshot → `05` vue de contrôle) ; `CONV-7` (normalisation BCP-47 / ISO 3166-1 α-2) est le **prérequis dur** des autres passes et forme un paquet indissociable avec sa mise à niveau frontend. **Non automatisable, à ne pas « finir »** : trois tables de revue (`conv_backup.titres_a_revoir_*`, `autorites_casse_a_revoir_*`, `autorites_patronyme_a_revoir_*`) sont créées **vides de tout verdict** — `initcap()` casse `Van der Walt`, `McKay`, `D'Amico`, et l'heuristique de double patronyme se trompe sur 14 % des cas (Mechoso, Borges, Marcos). Elles sont le plan de travail de l'**Atelier autorités**, pas un reste à liquider. Constat : `journal/audits/AUDIT_conventions_catalographiques_2026-08-20.md`.
