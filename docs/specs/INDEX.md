...
- **`spec-outils-federalistes`** (face fédération) — **annoncée** : bloc « Ferramentas federalistas » de la nav (entre `biblioteca` et `rede`), premier outil = l'annuaire des cercles ouverts (vue 1ʳᵉ personne : lecture membres / action coordenador). Foyer : **registre §`FED`** + cadrage `CADRAGE_modele_acces_concentrique_2026-06-04.md`. Modèle d'accès concentrique figé (FED-1..7) ; remplissage à venir.
