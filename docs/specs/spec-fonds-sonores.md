...
4. **P3 — UI Catalogação + OPAC.** **P3a backend ✅ livré** (migration `20260621205119`) : RPC `api.audio_resource_set_fingerprint` (persiste l'empreinte client) + `api.audio_tracklist_public` (tracklist OPAC anon-safe via `catalog_list_anon_v1`, FS-D9). **P3b ✅ livré** (composant `AudioSegmentsBlock` greffé dans `BookDraftForm` : segments + crédits + **picker d'œuvre complet**, i18n ×10, build/test verts). **P3d ✅ livré** (tracklist sur la fiche publique `BookPage` via `api.audio_tracklist_public` public-safe, sans nouvelle clé i18n). **P3c ✅ livré + round-trip PROUVÉ LIVE** (Woody Guthrie « Two Good Men (Sacco and Vanzetti) » @ 98 % : fichier → empreinte wasm `@unimusic/chromaprint` (wrapper `lib/chromaprintFingerprint.js` sans top-level await) → EF → AcoustID → bon MBID). « Appliquer » sur **segment** (`audio_track_set_recording_mbid`) ou ressource audio. *(Apply-to-segment = migration `20260622112833`, push en attente de la résolution du drift signup.)*
5. **P4 — i18n** ✅ **livré** : libellés segments + rôles réutilisés (P3b) + **erreurs `error.audio.*`** (11 clés × 10 + `localizeError` Cas 1b).
6. **P5 — Interop** ✅ **livré** : `fn_oai_harvestable_records` expose un champ `musicbrainz` (MBID artistes + enregistrements) → sérialisé en `<dc:relation>` (oai_dc) + `024 $2 musicbrainz` (marcxml). Migration `20260622120319` + `_shared/oai/metadata.ts`.
