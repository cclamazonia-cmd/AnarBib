...
| **spec-profils-bibliotheque.md v0.7** *(19/05/2026, clôture chantier)* | v0.7 | Doctrine compatible : une biblio en `circulation_mode = off` n'a pas d'onglet Consultations dans son painel. Une biblio en `informal` peut avoir des consultations simples sans cotisations. Une biblio en `full_sigb` a le workflow complet. ~~**Vérifié en prod** : BLMF (`full_sigb`), BTL (`informal`), BLT-test (`informal`)~~ — **relevé faux, corrigé le 30/08/2026** (item `J3`). Les trois affirmations l'étaient : `BLT-test` n'existe pas en base, et la BTL est en `full_sigb`. **État réel au 30/08/2026, relevé sur `public.libraries` :** les **cinq** bibliothèques — `blmf`, `blmf-teste`, `btl`, `cira-marseille`, `mleg` — sont **toutes** en `circulation_mode = full_sigb`, toutes actives, toutes en `governance_mode = full_governance`. **Aucune bibliothèque n'est en `informal` ni en `off`.** La doctrine ci-dessus reste juste — elle décrit ce que le produit fait selon le profil — mais son illustration décrivait une diversité de profils qui n'existe pas : les comportements adaptatifs `informal` et `off` n'ont donc **jamais été éprouvés sur une bibliothèque réelle**, contrairement à ce que « validés au paquet E.0-E.5 » laissait entendre. Ce qui a été validé l'a été sur des bascules de test, pas sur un usage. |
