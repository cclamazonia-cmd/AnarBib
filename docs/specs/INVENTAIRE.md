...
> **✅ Resync 16/06/2026 (vérif prod, lecture seule — MCP `execute_sql` + câblage frontend dans le repo).** Plusieurs specs marquées 🟡/🟠/⚠️ ont **dépassé leur statut** (souvent leur propre en-tête aussi). Vérifié objet par objet :
> - **`spec-cycle-vie-peb`** → **implémentée en prod** : 13 `fn_peb_*` + cron, tables `interlibrary_loans_v2`/items/events/transitions, **vues UI** `interlibrary_loans_painel_ui`/`_reports_ui`/`peb_history_v1`, frontend `PebHistorySection` (BibliotecaPage). Audit UX OK → **🔵**. *(L'en-tête de la spec dit encore « à implémenter » : à corriger.)*
> - **`spec-importacoes-exportacoes`** → **front+back en prod** : ~22 `fn_import_*` + **6 vues `partner_catalog_import_*_ui`**, pages `ImportacoesPage` + `ImportWizard`. La « parité UX à auditer » (⚠️ 10/06) est **levée** → **🟢** (finitions par lots résiduelles).
> - **`spec-partenariat-biblios`** → **front+back en prod** : `fn_partnership_*` (propose/accept/refuse/break/consent/sync_rights/sync_status/transparence) + tables `library_partnerships`/`partnership_rights`/`reader_partnership_consent`/`partnership_break_log` + vue `library_partnerships_ui`. **Reversée au corpus actif → 🟢** *(en-tête 🟡 « cadrée » = stale).*
> - **`spec-historico-retencao-lectrice`** → **C.5/C.6 déployés** : `fn_get/set_retention_policy` + `fn_get/set_my_retention_preference`, tables `library_retention_policies` + `user_history_retention_preferences`. Plus « en attente de déploiement » → **🟢**.
> - **`spec-oai-provider-gouvernance`** → **en prod** : `fn_oai_*` (propose/vote/close/decide/harvestable…) + tables `oai_opening_requests`/`_votes` + frontend `OaiSourcePanel` (rede). En-tête « livrés non poussés » = stale → **🟢 déployé**.
> - **`spec-catalogue-decouverte`** → **OPAC7 + OPAC9 livrés** : `api.catalog_facets_v1` (facettes à compteurs) + table `user_wishlist` (favoris serveur OPAC-W1), **câblés dans `CatalogPage`** (l.474/625). Restent OPAC8 (nuage sujets) / OPAC10 (parcours) / OPAC11 (RSS, différé anti-tracking) → **🟠 partiel**.
> - **`spec-atelier-autorites`** → **largement en prod** : `fn_authority_*` (propose/object/list/apply/withdraw/resolve_due) + tables `authority_proposals`/`_objections`/outbox + page `AtelierAutoridadesPage` + `SubjectAuthorityPicker`. En-tête 🟡 « préalables non levés » = stale → au moins **🟠**, probablement 🟢 sur le cœur *(à arbitrer)*.
> - **`spec-notify-prorrogacao-granulaire`** → en-tête de la spec = **🟢 livré 30/05** ; l'entrée détaillée INVENTAIRE (🟡 cadrage) est **stale** → **🟢**.
> - **`spec-onboarding-biblioteca-v2.0`** → **bien plus avancée que « #111 Q3 2026 »** : page `AtelierConstituicaoPage` (« Oficina de constituição » §6, routée `/atelier`, gardée `coordenador_em_constituicao`) + `fn_constitution_*` ×5 (complete/guard/set_profile/set_regimento/set_volet) + **9/10 volets câblés** (`WIRED_VOLETS = {1..9}` ; composants `AtelierVolet1/2/4/6/7` + éditeur générique) + PDF regimento. Volet 0 + volets 1-9 **en prod** → **🟠 (cœur livré)**, loin du 🟡 « à implémenter ».
> - **Confirmé NON implémenté (INVENTAIRE correct)** : **`spec-outils-federalistes`** (aucun objet `círculo` en base → reste **🟡 cadrée**) ; specs 🟡 onboarding-criar-conta / cartographie / renouvellement-granulaire / sources-externes-autorites (pas de preuve prod recherchée — laissées en l'état).
> - **Couche autorité — attribution démêlée (16/06)** : `fn_authority_*` + `authority_proposals`/`_objections` + `network_contributors` + `AtelierAutoridadesPage` = **`spec-atelier-autorites`** (migrations `20260613150100`/`20260614140000`, paquet 1) ; EF `authority_lookup`/`author_portrait_lookup` = **`spec-sources-externes-autorites`** ; `AuthorPage` enrichie = **`spec-notice-autorite-enrichie`**. En-têtes des 3 specs corrigés en conséquence.
> - **Récolement (inventaire par scan) — sous-système MOBILE livré SANS spec dédiée (16/06)** : tables `recolement_sessions`/`recolement_scans` + 4 RPC `api.recolement_*` (start/scan/finish/open_sessions, migration `20260615214324`) + onglet Painel `TabRecolement` (commit `b7274838`). Pas de fiche au corpus pour l'instant → tracé dans [`ETAT-AVANCEMENT`](../backlogs/ETAT-AVANCEMENT-multisessions.md) + [guide entraide](../guides/guide-scan-qr-pt-BR.md). **À doter d'une spec si le sous-système grossit.**
...
spec-autorites-notes-bio-multilingues v0.2 (socle Ateliers — author_translations étendue, ✅ socle livré 05/06)
  ├──→ spec-sources-externes-autorites (autorité multilingue, variant_forms = ancre Track D)
  └──→ spec-atelier-autorites (face contribution, en aval)

spec-liaison-autorites-oeuvres v0.2 (book_contributors.author_id ↔ book_authors, ✅ rétroactif livré 05/06)
  ├──→ spec-catalogacao-fiche-et-paliers (contributeurs saisis au catalogage)
  └──→ spec-notice-autorite-enrichie (liens auteur cliquables sur la notice, en aval)

spec-doublons-detection-fusion v0.1 (merge_log + merge_author/merge_book, ✅ livré 05/06)
  ├──→ spec-liaison-autorites-oeuvres (réutilise fn_normalize_name + trigramme)
  └──→ spec-exemplaires-circulation (merge_book déplace holdings/exemplaires, repointe la circulation)
...
spec-conventions-catalographiques v0.1 (conventions d'écriture des points d'accès et des titres)
  ├──→ spec-sources-externes-autorites (couche autorité, variant_forms CAT-D4/D6 — les formes rejetées y vivent)
  ├──→ spec-notice-autorite-enrichie (INV-4, D7 — collectivités et exports #AUT3, en aval)
  ├──→ archive/spec-catalogacao-fiche-et-paliers (champs et paliers de saisie — l'assistant §7.1 s'y greffe)
  ├──→ spec-catalogue-decouverte (#OPAC10 parcours alphabétique — consomme books.title_nonfiling, en aval)
  └──→ spec-atelier-autorites (les 3 tables de revue sont son plan de travail, en aval)
```
...
### 📐 `spec-conventions-catalographiques.md` *(nouveau v0.1 20/08)*

**Domaine** : Conventions d'écriture des points d'accès (autorités personne et collectivité) et des titres — casse, ordre des éléments, particules, suffixes de filiation, articles non-classants, dates incertaines, référentiels de langue et de pays.
**Version actuelle** : v0.1 (20 août 2026 — cadrage initial)
**Statut** : 🟡 Cadrée, **non implémentée**. `DOC-CONV-1` inscrit au **registre §0**, `CONV-1…CONV-7` + `CONV-O1…O4` au **§37 `CONV`** (registre v0.2, 20/08). Six migrations écrites, **aucune appliquée**.
**Rapport de constat** : [`AUDIT_conventions_catalographiques_2026-08-20.md`](../journal/audits/AUDIT_conventions_catalographiques_2026-08-20.md) — staging `uflwmikiyjfnikiphtcp`, 1 300 autorités / 2 674 notices, lecture seule.

**Périmètre** :
- **Doctrine `DOC-CONV-1`** : une seule vérité en base, plusieurs rendus. Le **point d'accès** (`sort_name`) n'est ni la **forme d'affichage** (`preferred_name`) ni la **forme d'export** (capitales ABNT). Les trois coexistent légitimement ; ce qui est illégitime, c'est de les confondre dans une même colonne.
- **`CONV-1/2`** — point d'accès en casse naturelle ; `sort_name` fait foi, `preferred_name` en dérive (jamais l'inverse).
- **`CONV-3`** — la casse des titres est pilotée par la **langue du titre**, pas par une règle universelle (l'allemand capitalise ses substantifs : c'est son orthographe, pas un artefact d'import).
- **`CONV-4`** — l'article non-classant est porté par `books.title_nonfiling` (offset, à la manière de l'indicateur 2 de la zone MARC 245) ; **le titre n'est jamais mutilé** pour les besoins du classement.
- **`CONV-5`** — dates = entiers + qualificatif (`exact | circa | uncertain | unknown | living`) ; EDTF différé mais projection sans perte préservée. Distingue enfin « encore vivant·e » de « date inconnue ».
- **`CONV-6`** 🟡 — `authors.name_lang` (BCP-47), **distinct de `country`** : c'est la langue du *nom* qui décide de la règle d'entrée, pas le pays de naissance (un·e Argentin·e peut porter un nom italien).
- **`CONV-7`** — normalisation `books.idioma` (BCP-47) et `authors.country` (ISO 3166-1 α-2). **Prérequis dur** des autres passes : la règle de casse des titres dépend de la langue.
- **Hors périmètre** : autorités matière, vedettes de collection et titres uniformes, CDD/indexation matière, résorption du champ libre `books.autor` (`CONV-O3`).

**Dépendances entrantes** :
- `spec-sources-externes-autorites` v0.2 (couche autorité, `variant_forms` CAT-D4/D6 — les formes rejetées y vivent)
- `spec-notice-autorite-enrichie` (INV-4, D7)
- `archive/spec-catalogacao-fiche-et-paliers` v0.4 (champs et paliers de saisie — l'assistant §7.1 s'y greffe)
- `REGISTRE_decisions.md` (`DOC-I18N-1`, `DOC-DEPLOY-1/3`, `DOC-ADDR-1`, `CAT-B1`)

**Dépendances sortantes** :
- `spec-catalogue-decouverte` (**#OPAC10** parcours alphabétique — consomme `books.title_nonfiling`)
- `spec-notice-autorite-enrichie` (**#AUT3** exports, D2 — la forme ABNT s'y calcule)
- `spec-atelier-autorites` (les trois tables de revue *sont* son plan de travail)

**Chantiers liés** :
- Chantier conventions — **non démarré en base**. Six migrations rédigées hors dépôt (`00` snapshot → `05` vue de contrôle permanente), à renuméroter (l'horodatage `20260820T1200` se placerait avant 13 migrations existantes ; dernière en date `20260821030000`) et à pousser paquet par paquet.

**⚠️ Points d'attention** *(relecture de reprise 20/08, actualisée après le déploiement du 20-21/08)* :

> **Celui qui a effectivement pété — `NOT VALID` ne dispense pas les écritures.** Les deux CHECK de la `01` étaient posées `NOT VALID` « pour ne bloquer aucune saisie en cours ». `NOT VALID` ne dispense que le **scan initial des lignes existantes** ; toute ligne insérée ou mise à jour ensuite est vérifiée. La rétro-qualification de dates de la `02` (574 autorités, sur une colonne sans rapport avec le pays) a donc fait exploser `authors_country_iso_chk` sur une ligne au `country` non soldé — transaction annulée, `db push` arrêté, `03` à `05` jamais parties. Corrigé par la migration `06`, qui solde le résidu (endonymes `Angleterre`, `Nederland`, `Türkiye`, absents de la table de correspondance) puis **valide** les contraintes. Règle : on solde le résidu avant de poser la contrainte, ou on ne la pose pas.

1. **Les migrations `00` et `03` rendent la CI rouge en l'état.** `scripts/ci/run-sql-suites.sh` reconstruit une base **vide** et applique toutes les migrations avec `ON_ERROR_STOP=1`. Or `00` lève `raise exception 'Snapshot vide'` et `03` lève `'C suspecte : 0 proposition'` — précisément ce que produit une base sans données. Même classe de piège que le garde-fou `cron.schedule`. Les gardes doivent être conditionnés à la présence de données, pas à leur absence.
2. **La vue de contrôle de `05` expose l'annuaire des autorités.** `grant select … to authenticated` sur une vue **sans `security_invoker`** = tout compte inscrit de la fédération lit `sort_name`, `preferred_name`, `country` et les titres, RLS contournée. Reproduit, un cran plus bas, le trou des fonctions `SECURITY DEFINER` ouvertes à `anon`. → patron `SECURITY DEFINER` gaté staff, vue en `private`. *(À vérifier au passage : la branche `T1` appelle `fn_conv_lower_stopwords`, dont `03` révoque l'`EXECUTE` à `authenticated` — le droit sur une fonction se vérifie contre l'appelant, pas contre le propriétaire de la vue.)*
3. **`04` section C.2 dépasse ses « 68 fiches »** : le prédicat attrape *toute* fiche dont `preferred_name` contient une virgule, sans vérifier qu'il s'agit bien de la forme inversée — collectivités, qualificatifs, suffixes compris. Ajouter `and preferred_name = sort_name`, et découper sur la **première** virgule seulement (`split_part(…, 2)` perd la queue des `sort_name` à deux virgules).
4. **Le chemin d'écriture n'est pas normalisé** : `book_drafts.idioma` et `author_drafts.country` sont hors périmètre de `01` (un brouillon publié après coup ré-injecte l'ancien régime) ; `AuthorDraftForm.jsx` saisit le pays en texte libre avec le placeholder `Brasil` ; `heuristics.js` mappe l'OCR portugais sur `'pt'` et non `'pt-BR'`. **Tant que ces trois portes sont ouvertes, les `VALIDATE CONSTRAINT` finaux ne pourront jamais être joués** — ce qui vide de sens les contraintes `NOT VALID` de `01`.
5. **`CONV-7` et sa mise à niveau frontend forment un paquet indissociable** : `BookPage.jsx` et `WorkPage.jsx` affichent `idioma` **brut** (« pt-BR » au lieu de « Português » ; « Pt-br » en en-tête de groupe d'expression) et le filtre langue du catalogue est un **champ texte libre** (`catalogFilters.js`, `api.catalog_facets_v1`) — un lecteur qui tape « Português » obtiendra zéro résultat. Côté `country` en revanche, `getCountryName()` accepte déjà code ISO **et** nom en clair : l'affichage est immunisé.
6. **`04` casse le tri de l'Atelier autorités** : `CatalogPanel.jsx` ordonne sur `preferred_name`, qui passera de « SILVEIRA, Ênio » à « Ênio Silveira » — la liste triera **par prénom**, sur l'écran même qui sert à dépouiller les tables de revue. Correctif d'une ligne (`.order('sort_name')`), et il *incarne* `CONV-2`.
7. **Le filet de sécurité n'est dans aucune sauvegarde** : `anarbib-bg2.sh` fait `pg_dump --schema=public` ; le schéma `conv_backup`, dont toute la raison d'être est la réversibilité, n'est dans aucun des trois flux restic. Après restauration, rejouer `00` ne photographierait que des données déjà corrigées. *(En revanche il échappe légitimement au filet BG2 de la CI, qui ne balaie que `public` : rien à ajouter à `deploy/bg2-known-tables.txt`.)*
8. **Plusieurs blocs de vérification transforment une condition de données en blocage de déploiement** (`03` articles rejetés, `04` particules et suffixes) : une seule fiche éditée depuis l'audit, ou une seule notice saisie dans l'ancien style, fait avorter la migration et bloque la file Woodpecker. → `raise warning` avec la liste des identifiants fautifs, en réservant `raise exception` aux invariants structurels.
9. **`01` laisse des `work_expressions` orphelines** (`português`, `espanhol`…) qu'aucune notice ne référence plus une fois le trigger `trg_sync_book_expression` passé — d'où de possibles groupes de langue vides sur `WorkPage`.
10. **`CONV-4` n'est pas encore consommable** : la colonne et son index sont créés, mais aucune vue du catalogue n'expose `title_nonfiling` — #OPAC10 trie toujours sur `titulo.asc`. La migration prépare le terrain, elle ne livre pas la fonctionnalité.
11. **Trois tables de revue à ne surtout pas « finir »** : `conv_backup.titres_a_revoir_20260820` (~216 propositions de casse — la fonction retire l'artefact d'import, elle ne décide pas si un mot est un nom propre), `autorites_casse_a_revoir_20260820` (`initcap()` casse `Van der Walt`, `McKay`, `D'Amico`, `Ramón y Cajal`), `autorites_patronyme_a_revoir_20260820` (22 doubles patronymes, **3 faux positifs connus** — Mechoso, Borges, Marcos — soit 14 % d'erreur). Elles sont créées **vides de tout verdict** ; le SQL d'application reste en commentaire, avec garde anti-écrasement. Aucun script ne doit passer `valide = true` en masse : c'est le plan de travail de l'**Atelier autorités**, pas un reste à liquider.
...
- ✅ **`spec-flux-partage-numerique`** (#ILL-digital) : **implémentée & déployée le 13/06** (lots ILL-I1..I5 ; spec passée 🟢). **Vérifié en prod le 16/06** : tables `ill_digital_shares` + `ill_digital_share_events`, **7 RPC `fn_ill_*`** (request/respond/start_digitization/transmit/acknowledge/close/signed_url), migration `20260612230500` appliquée ; EF `read-ill-shared-asset` + `notify-digital-share` ; **frontend `LibraryDigitalSharesSection`** (BibliotecaPage onglet `ill`) ; i18n `digishare.*` ×10 (parité). **Parité UX = faite.** *(L'ancienne mention ⚠️ « backend livré, UX à auditer » datait du 10/06, **antérieure** à l'implémentation du 13/06 ; sa description `digital_assets`/`Exchange*` visait la couche* recursos digitais *réutilisée, pas le flux ILL lui-même.)* Reste **référence active 🟢** (sous-système dont dépend `spec-partenariat-biblios`) — pas un candidat `archive/`. Circuit distinct du PEB ; droit `PARTNER-D9`.
