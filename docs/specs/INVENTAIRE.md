...
> - **À regarder de plus près (ne pas reclasser à l'aveugle)** : attribution de la couche autorité (`fn_authority_*` + `authority_proposals`/`_objections`, backend commun en prod) entre `atelier-autorites` / `notice-autorite-enrichie` / `sources-externes-autorites` — quelle spec porte quoi reste à démêler avant reclassement fin.
