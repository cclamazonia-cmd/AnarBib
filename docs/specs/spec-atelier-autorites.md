...
| **Statut** | 🟠 **Paquet 1 implémenté en prod** *(MAJ 16/06/2026)* : tables `network_contributors`/`authority_proposals`/`authority_proposal_objections`, RPC `fn_authority_*` (propose/object/list/apply/withdraw/resolve_due) + cron + events/notif (migrations `20260613150100` + `20260614140000`), frontend `AtelierAutoridadesPage` + `SubjectAuthorityPicker`. Préalables (collectivité/matière, rôle contributeur `network_contributors`) **levés**. Restent ATE-O1/O4/O5 (création/traduction = apply différé). Cf. INVENTAIRE Resync 16/06. |
