-- publish_exemplar_draft : l'ancien filtre
--   where h.id = v_draft.target_holding_id
--     and (v_library_id is null or h.library_id = v_library_id)
-- degenerait en "vrai" quand v_library_id etait encore nul (cas normal d'un
-- brouillon neuf), et la ligne suivante ecrasait silencieusement v_library_id
-- par celle du holding trouve, quelle que soit sa biblioteca. Un
-- target_holding_id errone/reutilise pouvait donc rediriger tout l'exemplaire
-- vers une autre biblioteca sans la moindre erreur.
--
-- On ne rejette desormais que le cas d'un conflit explicite (biblioteca-cible
-- deja connue ET differente du holding vise) ; sinon on adopte la biblioteca
-- du holding comme avant. La garde-fou DB de la migration precedente
-- (trg_exemplar_library_matches_holding) reste la protection de dernier
-- recours si un autre chemin de code introduisait la meme incoherence.
