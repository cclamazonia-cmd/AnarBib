-- fn_healthcheck_notifications() : delai de grace de 15 min sur les files.
--
-- Constat (20, 21 et 22/09/2026) : chaque jour a 11:35 (Paris) le courriel
-- « un flux de notifications est casse » (files_non_traitees :
-- team_notification_outbox = 1), puis a 11:40 « incoherence resolue ».
-- Cause : le cron anarbib-team-invitations-remind (35 9 * * *) enfile une
-- ligne a 09:35:00 UTC ; la sonde (*/5) la lit a 09:35:02, avant que l'envoi
-- ne la passe en sent/skipped quelques secondes plus tard (lignes 116 et 118).
-- Une file n'est bloquee que si une ligne y reste : on ne compte plus que les
-- lignes non traitees creees il y a plus de 15 minutes (toutes les *outbox*
-- portent created_at, verifie en prod le 22/09).
--
-- Seule la rubrique 3 change ; le reste reprend la definition en prod
-- (identique a 20260817170941).
...
comment on function public.fn_healthcheck_notifications() is
  'Sonde de sante des notifications. Regle : `ok` ne compte QUE ce qui est casse ET que quelqu''un compte reparer (secrets Vault manquants sur un flux actif, event_type refuses par la CHECK, files bloquees depuis plus de 15 min). Secrets dormants et bibliotheques sans contact sont listes, motives, mais non bloquants. Lecture seule ; ne renvoie jamais la valeur d''un secret.';
