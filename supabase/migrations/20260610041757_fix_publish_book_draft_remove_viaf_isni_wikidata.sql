-- ════════════════════════════════════════════════════════════════════════════
-- Fix bug bloquant : publish_book_draft référençait viaf/isni/wikidata (inexistants)
-- Auteur  : Claude (Opus)
-- Session : Enrichissement données & backlog
-- Date    : 2026-06-10 (UTC)
--
-- SYMPTÔME : à l'enregistrement/publication d'une fiche document, erreur
--   « record "v_draft" has no field "viaf" » → impossible d'enregistrer.
--
-- CAUSE : 20260606184914_publish_book_draft_all_columns.sql avait ajouté
--   `viaf, isni, wikidata` à l'INSERT et à l'UPDATE de publish_book_draft, en
--   les lisant depuis v_draft (book_drafts%rowtype). Or ces colonnes n'existent
--   ni sur book_drafts, ni sur books : viaf_id/isni/wikidata_id sont des
--   identifiants d'AUTORITÉ (authors / author_drafts), pas des fiches document.
--   Le record v_draft n'ayant pas de champ `viaf`, la fonction plantait.
--
-- FIX : retirer viaf/isni/wikidata des 3 emplacements (colonnes INSERT, VALUES,
--   UPDATE). `subjects` est conservé (présent sur books et book_drafts).
--   Si un jour on veut ces identifiants sur les œuvres, il faudra d'abord créer
--   les colonnes sur books ET book_drafts AVANT de les référencer ici.
--
-- NB : déjà appliqué en prod en hotfix le 2026-06-10 ; cette migration persiste
--   l'état corrigé (CREATE OR REPLACE idempotent).
-- ════════════════════════════════════════════════════════════════════════════
