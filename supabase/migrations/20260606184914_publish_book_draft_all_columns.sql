-- =========================================================================
-- publish_book_draft — consolidation : toutes les colonnes dans la RPC
-- =========================================================================
-- Date     : 2026-06-06 (horodatage UTC reel)
-- Chantier : catalogacao — robustesse publication brouillon -> livre
-- Auteur   : Xavier + Claude
--
-- OBJET
-- -----
-- publish_book_draft copie explicitement chaque colonne de book_drafts vers
-- books. Les colonnes ajoutees recemment (distribuidora, tese/artigo/
-- relatorio/zine, subjects, viaf/isni/wikidata) ne transitaient que par le
-- trigger fn_propagate_circulation_default_on_publish (AFTER UPDATE OF
-- status). Ca fonctionne mais c'est fragile : si le trigger est desactive
-- ou modifie, ces colonnes sont perdues.
--
-- Ce script ajoute les 18 colonnes manquantes directement dans l'INSERT et
-- l'UPDATE de publish_book_draft. Le trigger reste en place (belt-and-
-- suspenders). Signature inchangee, ACL preservees (CREATE OR REPLACE).
-- =========================================================================
...
-- Verification : les 18 colonnes consolidees sont bien dans le corps
DO $verif$
BEGIN
  ...
  END IF;
  IF position('tese_university' IN
       pg_get_functiondef('public.publish_book_draft(bigint)'::regprocedure)) = 0 THEN
    RAISE EXCEPTION 'VERIF: tese_university absente de publish_book_draft';
  END IF;
...
-- (migration 20260603115326_p1_3a_publish_book_draft_garde_isbn.sql)
-- =========================================================================
