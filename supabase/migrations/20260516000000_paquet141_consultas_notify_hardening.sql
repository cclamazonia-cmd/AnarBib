...
-- Date : 2026-05-16
-- ============================================================
-- v3 (16/05/2026) : fix du DO-block 4.4 qui levait une fausse alarme
-- "trg_notify_consulta_lifecycle conserve un GRANT TO PUBLIC".
-- 
-- v2 utilisait : pg_catalog.array_to_string(p.proacl, ',') LIKE '%=X/%'
-- Probleme : ce pattern matche aussi les GRANTs explicites a des roles
-- nommes (postgres=X/, authenticated=X/, etc.), pas seulement PUBLIC.
-- 
-- v3 utilise : unnest(p.proacl) AS aclitem WHERE aclitem::text LIKE '=%'
-- Le '=' en debut de chaine = PUBLIC sans role prefixe (notation Postgres).
-- ============================================================
...
-- DOCTRINE SÉCURITÉ : REVOKE FROM PUBLIC (correction dette doctrinale paquet 26)
...
BEGIN
  ...
  -- Transition schedule_reply_status → confirmado / recusado (existant)
  IF TG_OP = 'UPDATE'
  ...
  IF v_emit_em_preparacao THEN
    ...
    IF v_enabled THEN
      ...
      PERFORM public.fn_dispatch_circulation_notify_event(
        v_event, NEW.consulta_id, v_payload
      );
    END IF;
  END IF;
...
-- ============================================================
-- SECTION 4 — Vérification finale (DO-block v3)
-- ============================================================
-- v3 (16/05) : fix pattern de détection GRANT TO PUBLIC.
-- Avant : LIKE '%=X/%' matchait aussi authenticated=X/, postgres=X/, etc.
-- Après : unnest() + LIKE '=%' matche uniquement le '=X/' initial (PUBLIC).
-- ============================================================
...
DECLARE
  ...
  v_lifecycle_public_count int;
  v_workflow_public_count int;
BEGIN
  -- 4.1 Colonnes toggle existent
  SELECT EXISTS (
  ...
  -- 4.2 Propagation workflow_note dans lifecycle
  ...
  -- 4.3 Events présents dans workflow
  ...
  -- 4.4 v3 : PUBLIC n'a plus EXECUTE (pattern PRÉCIS via unnest)
  -- En Postgres, un acl item qui commence par '=' = GRANT TO PUBLIC.
  -- Un acl item préfixé par un rôle (ex 'authenticated=X/') = GRANT TO ce rôle.
  
  SELECT count(*) INTO v_lifecycle_public_count
  FROM pg_proc p
  JOIN pg_namespace n ON p.pronamespace = n.oid,
       unnest(p.proacl) AS aclitem
  WHERE n.nspname = 'public' 
    AND p.proname = 'trg_notify_consulta_lifecycle'
    AND aclitem::text LIKE '=%';
  
  IF v_lifecycle_public_count > 0 THEN
    RAISE EXCEPTION 'Verification echouee : trg_notify_consulta_lifecycle conserve un GRANT TO PUBLIC';
  END IF;
  
  SELECT count(*) INTO v_workflow_public_count
  FROM pg_proc p
  JOIN pg_namespace n ON p.pronamespace = n.oid,
       unnest(p.proacl) AS aclitem
  WHERE n.nspname = 'public' 
  ...
  IF v_workflow_public_count > 0 THEN
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Paquet 141.1 applique avec succes (16/05/2026)';
