...
async function handleNotifyEvent(req) {
  ...
  try {
    ...
    if (!event) return jsonResponse(400, {
      ok: false,
      error: "Invalid event"
    });
    if (!Number.isFinite(recordId) || recordId <= 0) return jsonResponse(400, {
      ok: false,
    ...
    return jsonResponse(200, {
      ...
      resolved_admin_email: ADMIN_EMAIL,
      ...result
    });
  } catch (error) {
