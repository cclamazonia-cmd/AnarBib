...
import { handleBugReportEvent } from "../domain/bug-report.ts";
...
async function dispatchInterne(event, recordId, payload) {
  ...
  // Events bug_report.* (« Signaler un problème », E14, 24/09/2026) - handler dédié, lit
  // bug_report_notification_outbox par recordId. received -> admins réseau (+ accusé de réception).
  if (event.startsWith("bug_report.")) return await handleBugReportEvent(recordId);
