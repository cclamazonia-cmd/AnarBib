...
export async function dispatchNotifyEvent(event: string, recordId: number, payload?: NotifyPayload | null): Promise<Record<string, unknown> | null> {
  ...
  if (["emprestimo_v2_criado","emprestimo_v2_prorrogado","emprestimo_prorrogado","emprestimo_v2_devolvido","emprestimo_v2_parcialmente_devolvido","emprestimo_v2_devolvido_apos_parcial"].includes(event)) return await handleEmprestimoV2(recordId, event === "emprestimo_prorrogado" ? "emprestimo_v2_prorrogado" : event);
