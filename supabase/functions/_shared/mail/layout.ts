...
export function footerPadrao(ctx) {
  const p = [];
  if (REGIMENTO_URL) p.push(`Regimento: <a href="${esc(REGIMENTO_URL)}" style="color:#fff;text-decoration:underline;">abrir</a>`);
  if (LIBRARIAN_PHONE) p.push(`Telefone: <b>${esc(LIBRARIAN_PHONE)}</b>`);
  if (ctx?.footer_local) p.push(String(ctx.footer_local).trim());
  else p.push("Em caso de dúvida, entre em contato com a biblioteca.");
  return p.join("<br>");
}
export function renderEmail(opts) {
  ...
  tl.push("");
  if (REGIMENTO_URL) tl.push(`Regimento: ${REGIMENTO_URL}`);
  if (LIBRARIAN_PHONE) tl.push(`Telefone: ${LIBRARIAN_PHONE}`);
  tl.push(FOOTER_TEXT);
  return {
    html,
    text: tl.join("\n")
