...
export async function handleReservaCriadaOld(recordId) {
  ...
  const user = {
    email: String(r.user_email || r.email || "").trim(),
    name: firstNameOnly(r.first_name) || firstNameOnly(userFullName) || undefined
  };
...
export async function handleEmprestimoOld(recordId, event) {
  ...
  const user = {
    email: String(e.user_email || e.email || "").trim(),
    name: firstNameOnly(userFullName) || undefined
  };
  const adminUserName = adminDisplayName(userFullName, user.email);
  ...
  if (event === "emprestimo_criado") {
    subject = `Empréstimo registrado — ${brandTag}`;
    title = "Empréstimo registrado";
    introHtml = `<p style="margin:0 0 10px;">Seu <b>empréstimo</b> foi registrado.</p>${dueDate ? `<p style="margin:0;">Devolução prevista: <b>${esc(formatDateBR(dueDate))}</b>.</p>` : ""}`;
  } else if (event === "emprestimo_prorrogado") {
    subject = `Prorrogação confirmada — ${brandTag}`;
    title = "Prorrogação confirmada";
    introHtml = `<p>Sua <b>prorrogação</b> foi confirmada.</p>${dueDate ? `<p>Nova devolução: <b>${esc(formatDateBR(dueDate))}</b>.</p>` : ""}`;
  } else if (event === "emprestimo_devolvido") {
    subject = `Devolução registrada — ${brandTag}`;
    title = "Devolução registrada";
    introHtml = `<p>Registramos a <b>devolução</b>. Obrigado!</p>`;
  } else if (event.startsWith("lembrete_") || event.startsWith("aviso_")) {
    subject = `${title = event.includes("atraso") ? "Aviso de atraso" : "Lembrete de devolução"} — ${brandTag}`;
  ...
  if (isLifecycle && loanLifecycleEnabled(ctx) && loanAdminCopyEnabled(ctx)) {
    adminResult = await sendAdminNotification({
      context: ctx,
      subject: applyBrandingText(`[${brandTag}] ${title} — ${adminUserName}`, ctx),
      title,
      introHtml: `<p>Atualização de empréstimo (sistema antigo).</p>`,
      details: [
        {
          label: "Leitor(a/e)",
          value: adminUserName
