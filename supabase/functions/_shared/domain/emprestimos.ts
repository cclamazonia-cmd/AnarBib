...
export async function handleEmprestimoV2(recordId:number,event:string) {
  ...
  let detKeys:Array<{key:string,value:string}>=[];
  if (event==="emprestimo_v2_criado") { const t=joinTitles(oi.map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); sub=`${tMail(locale,"loan.created.sub")} — BLMF`; tit=tMail(locale,"loan.created.sub"); intro=`<p style="margin:0 0 10px;">${tMail(locale,"loan.created.intro")}</p>${da?`<p style="margin:0 0 10px;">${tMail(locale,"loan.dueIn",{date:esc(fmtD(da))})}</p>`:""}<p style="margin:0;">${tMail(locale,"layout.keepMsg")}</p>`; detKeys=[...(t?[{key:"items",value:t}]:[]),...(ca?[{key:"registration",value:fmtD(ca)}]:[]),...(da?[{key:"dueDate",value:fmtD(da)}]:[])]; }
  else if (event==="emprestimo_v2_prorrogado") { const t=joinTitles((oi.length?oi:items).map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); sub=`${tMail(locale,"loan.renewed.sub")} — BLMF`; tit=tMail(locale,"loan.renewed.sub"); intro=`<p style="margin:0 0 10px;">${tMail(locale,"loan.renewed.intro")}</p>${da?`<p style="margin:0 0 10px;">${tMail(locale,"loan.newDue",{date:esc(fmtD(da))})}</p>`:""}<p style="margin:0;">${tMail(locale,"loan.renewed.once")}</p>`; detKeys=[...(t?[{key:"items",value:t}]:[]),...(da?[{key:"newDueDate",value:fmtD(da)}]:[]),...(ea?[{key:"renewal",value:fmtD(ea)}]:[])]; }
  else if (event==="emprestimo_v2_devolvido") { const t=joinTitles((ri.length?ri:items).map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); const ra=ri.find(i=>i.returned_at)?.returned_at; sub=`${tMail(locale,"loan.returned.sub")} — BLMF`; tit=tMail(locale,"loan.returned.sub"); intro=`<p style="margin:0 0 10px;">${tMail(locale,"loan.returned.intro")}</p><p style="margin:0;">${tMail(locale,"loan.returned.browse")}</p>`; detKeys=[...(t?[{key:"items",value:t}]:[]),...(ra?[{key:"return",value:fmtD(String(ra))}]:[])]; }
  else if (event==="emprestimo_v2_parcialmente_devolvido") { const tReturned=joinTitles(ri.map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); const tRemaining=joinTitles(oi.map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); const remainingDueAt=oi.reduce((acc:string,i)=>{const d=String(i.extended_until||i.due_at||"").trim();return d&&(!acc||d>acc)?d:acc;},""); sub=`${tMail(locale,"loan.partialReturn.sub")} — BLMF`; tit=tMail(locale,"loan.partialReturn.sub"); intro=`<p style="margin:0 0 10px;">${tMail(locale,"loan.partialReturn.intro")}</p>${remainingDueAt?`<p style="margin:0 0 10px;">${tMail(locale,"loan.partialReturn.dueReminder",{date:esc(fmtD(remainingDueAt))})}</p>`:""}<p style="margin:0;">${tMail(locale,"loan.partialReturn.outro")}</p>`; detKeys=[...(tReturned?[{key:"itemsReturned",value:tReturned}]:[]),...(tRemaining?[{key:"itemsRemaining",value:tRemaining}]:[]),...(remainingDueAt?[{key:"dueDate",value:fmtD(remainingDueAt)}]:[])]; }
  else if (event==="emprestimo_v2_devolvido_apos_parcial") { const tAll=joinTitles((ri.length?ri:items).map(i=>String(i.titulo||`[${String(i.bib_ref||"").trim()}]`))); const ra=ri.find(i=>i.returned_at)?.returned_at; sub=`${tMail(locale,"loan.fullyReturnedAfterPartial.sub")} — BLMF`; tit=tMail(locale,"loan.fullyReturnedAfterPartial.sub"); intro=`<p style="margin:0 0 10px;">${tMail(locale,"loan.fullyReturnedAfterPartial.intro")}</p><p style="margin:0;">${tMail(locale,"loan.fullyReturnedAfterPartial.browse")}</p>`; detKeys=[...(tAll?[{key:"items",value:tAll}]:[]),...(ra?[{key:"return",value:fmtD(String(ra))}]:[])]; }
  else throw new Error(`Evento não suportado: ${event}`);
  // Derive det (locale du lecteur) et adminDet (force pt-BR) depuis detKeys
  const det:EmailDetails=detKeys.map(k=>({label:label(locale,k.key),value:k.value}));
...
export async function handleEmprestimoDevolucaoEvent(recordId:number,event:string,payload?:NotifyPayload|null) {
  ...
  const detKeys2:Array<{key:string,value:string}>=[...(tits?[{key:"items",value:tits}]:[]),...(when&&event!=="emprestimo_devolucao_cancelada"?[{key:"dueDate",value:when}]:[]),...(da?[{key:"deadline",value:da}]:[])];
  const det:EmailDetails=detKeys2.map(k=>({label:label(locale,k.key),value:k.value}));
  const {html,text}=renderEmail({preheader:tit,title:tit,greeting:greeting(locale,user.name),introHtml:intro,details:det,footerHtml:footerPadrao(ctx),context:ctx});
