...
import { footerPadrao, renderEmail, type ActionBox } from "../mail/layout.ts";
...
export async function handleReservaV2WorkflowEvent(recordId:number,event:string,payload?:NotifyPayload|null) {
  ...
  let readerKey:string|null=null, staffKey:string|null=null, staffMailEnabled=true, staffNeedsAction=false;
  if (we==="retirada_a_combinar") {
    if (proposedBy==='biblio') {
      if (iterCount===0) { readerKey='wf.reader.libraryProposed'; staffKey='wf.staff.negotiationOpened'; }
      else { readerKey='wf.reader.libraryCounterProposed'; staffKey='wf.staff.staffCounterProposed'; }
    } else if (proposedBy==='leitor') {
      readerKey='wf.reader.youCounterProposed'; staffKey='wf.staff.readerCounterProposed'; staffNeedsAction=true;
    } else {
      ...
      readerKey='wf.toCoordinate'; staffKey=null; staffMailEnabled=false;
    }
    ...
    readerKey='wf.pickupRescheduled'; staffKey=null; staffMailEnabled=false;
  } else if (we==="pronta_para_retirada") {
    readerKey='wf.ready'; staffKey='wf.staff.ready';
  } else if (we==="retirada_no_show") {
    readerKey='wf.noShow'; staffKey='wf.staff.noShow';
  } else if (we==="liberada_para_circulacao") {
    readerKey='wf.closed'; staffKey='wf.staff.closed';
  ...
  let ar; if (!staffMailEnabled||!staffKey) {
