// supabase/functions/lettre-unsubscribe/index.ts
...
Deno.serve(async (req) => {
  ...
  const { data, error } = await sb.schema("api").rpc("fn_lettre_unsubscribe", { p_token: token });
  if (error) return page(locale, "lettre.landing.error", 500);
  const status = String(data || "");
  if (status === "unsubscribed") return page(locale, "lettre.landing.unsubscribed");
