...
- **Backend** — l'état chiffré (fonctions Edge, migrations appliquées, tables, policies) vit dans [`docs/backlogs/`](docs/backlogs/), qui en porte une **photo datée** et dit d'où vient chaque nombre. Il n'est plus recopié ici, et la raison tient en une observation : la ligne qu'occupait cette place annonçait « 224 migrations, la dernière étant `20260830180000` », **recomptées en base le matin du 30/08/2026**. Le soir du même jour, la base en portait **231**, la dernière étant `20260830210000`. Un chiffre juste au moment où on l'écrit devient faux dans la journée, et rien dans un README ne signale qu'il a vieilli — c'est le même défaut que celui du titre de cette section, à un étage de plus. Un renvoi ne périme pas ; une copie, si.
